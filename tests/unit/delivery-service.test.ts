...
class FakeRepository implements DeliveryRepository {
  ...
  async transition(input: { status: DeliveryRecord['status']; expectedVersion: number }) {
    this.value = { ...this.value, status: input.status, versionNumber: input.expectedVersion + 1 };
    return this.value;
  }
  async attachReservationReferences() {
    return this.value;
  }
}
...
describe('DeliveryService', () => {
  it('creates and audits an idempotent delivery request', async () => {
    ...
    await expect(
      service.create(context, { salesOrderId: randomUUID(), idempotencyKey: 'request-1' }),
    ).resolves.toBeTruthy();
  ...
  it('fulfills all order reservations before completing a delivery', async () => {
    ...
    await expect(service.transition(context, repository.value.id, 'COMPLETED', 1)).resolves.toMatchObject({
      status: 'COMPLETED',
    });
        ...
        listReservationsBySource: async () => [
          {
            ...
            sourceId: repository.value.salesOrderId,
