...
describe('PricingService', () => {
  it('creates, audits, and validates price lists', async () => {
    const x = service();
    await expect(
      x.value.create(context, { code: 'STD', name: 'Standard', currency: 'INR', effectiveFrom: '2026-01-01' }),
    ).resolves.toBe(record);
    expect(x.audit.actions).toEqual(['pricing.created']);
  });
  it('rejects invalid item prices and denied access', async () => {
    ...
    const y = service();
    await expect(
      y.value.addItem(context, record.id, {
        itemCode: 'ITEM-1',
        unitOfMeasure: 'EA',
        price: -1,
        effectiveFrom: '2026-01-01',
  ...
  it('enforces module access', async () => {
