...
class FakeRepository implements ItemRepository {
  ...
  async update(): Promise<ItemRecord | null> {
    return this.item;
  }
  async softDelete(): Promise<ItemRecord | null> {
...
describe('ItemMasterService', () => {
  it('creates, reads, lists, updates, and soft-deletes items with audit events', async () => {
    ...
    await expect(
      service.create(context, { code: ' sku-1 ', name: ' Widget ', unitOfMeasure: 'EA' }),
    ).resolves.toBeTruthy();
    await expect(service.get(context, randomUUID())).resolves.toBeTruthy();
    await expect(service.list(context, { page: 1, pageSize: 20, search: 'widget' })).resolves.toMatchObject({
      total: 1,
    });
    await expect(
      service.update(context, randomUUID(), {
        name: 'Updated',
        unitOfMeasure: 'EA',
        salesEligible: true,
        expectedVersion: 1,
  ...
  it('rejects invalid item input and stale mutations', async () => {
    ...
    await expect(service.create(context, { code: ' ', name: 'Widget', unitOfMeasure: 'EA' })).rejects.toBeInstanceOf(
      ValidationError,
    );
  ...
  it('enforces the Inventory module and item permissions', async () => {
    await expect(createService({ moduleEnabled: false }).service.list(context, {})).rejects.toBeInstanceOf(
      ForbiddenError,
    );
