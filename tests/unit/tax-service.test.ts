...
class Repository implements TaxRepository {
  ...
  async resolve(): Promise<TaxRuleRecord | null> {
    return rule;
  }
}
class MissingRepository extends Repository {
...
describe('TaxService', () => {
  it('resolves deterministic tax amounts', async () => {
    await expect(service().calculate(context, { amount: 100, asOf: '2026-09-05' })).resolves.toMatchObject({
      rate: 18,
      taxAmount: 18,
    });
  });
  it('rejects missing effective rules and denied access', async () => {
    ...
    const missing = new TaxService(
      new MissingRepository(),
