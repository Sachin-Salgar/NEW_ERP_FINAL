...
class FakeRepository implements SalesReturnRepository {
  ...
  async transition(input: { status: SalesReturnRecord['status']; expectedVersion: number }) {
    this.value = { ...this.value, status: input.status, versionNumber: input.expectedVersion + 1 };
    return this.value;
  }
  async process(input: { expectedVersion: number }) {
    this.value = {
      ...this.value,
      status: 'PROCESSED',
...
describe('SalesReturnService', () => {
  it('creates and audits a return coordination record', async () => {
    ...
    await expect(
      service.create(context, { invoiceId: randomUUID(), idempotencyKey: 'request-1' }),
    ).resolves.toMatchObject({ inventoryStatus: 'NOT_CONNECTED', financeStatus: 'NOT_CONNECTED' });
  ...
  it('rejects an invalid lifecycle transition', async () => {
    ...
    await expect(service.transition(context, repository.value.id, 'CANCELLED', 1)).rejects.toBeInstanceOf(
      ValidationError,
    );
  });
  ...
  it('processes an approved return through one atomic repository operation', async () => {
      ...
      items: [
        {
          ...
          unitPrice: 2,
          itemId: randomUUID(),
          warehouseId: repository.value.warehouseId,
        },
      ],
    };
    const calls: string[] = [];
    ...
    const withInventory = new SalesReturnService(
      repository,
      { hasPermission: async () => true },
      { isModuleEnabled: async () => true },
      audit,
    ...
    await expect(withInventory.transition(context, repository.value.id, 'PROCESSED', 1)).resolves.toMatchObject({
      status: 'PROCESSED',
      inventoryStatus: 'COMPLETED',
      versionNumber: 2,
    });
