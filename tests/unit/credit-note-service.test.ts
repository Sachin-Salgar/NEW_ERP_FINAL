...
class FakeRepository implements CreditNoteRepository {
  ...
  async transition(input: { status: CreditNoteRecord['status']; expectedVersion: number }) {
...
describe('CreditNoteService', () => {
  it('creates and audits a not-connected credit note', async () => {
    ...
    await expect(
      service.create(context, { returnId: randomUUID(), idempotencyKey: 'request-1' }),
    ).resolves.toMatchObject({ financeStatus: 'NOT_CONNECTED', taxStatus: 'NOT_CONNECTED' });
