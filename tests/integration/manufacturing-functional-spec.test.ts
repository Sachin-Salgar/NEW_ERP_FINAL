...
describe('Manufacturing Functional Specification execution proof',()=>{
 ...
 it('executes capability -> routing -> work order -> task -> readiness -> production -> quality',async()=>{
  ...
  const item=await value.adminPool.query<{id:string}>(`SELECT id FROM inventory_items WHERE tenant_id=$1 AND is_deleted=false ORDER BY id LIMIT 1`,[value.tenantA.tenantId]);
  ...
  const process=(await value.app.inject({method:'POST',url:'/api/v1/manufacturing/process-details',headers:h,payload:{itemId:item.rows[0].id,revision:'CI-R1',status:'ACTIVE'}}));expect(process.statusCode).toBe(201);
