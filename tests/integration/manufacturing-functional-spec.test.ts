...
describe('Manufacturing Functional Specification execution proof',()=>{
 ...
 it('executes capability -> routing -> work order -> task -> readiness -> production -> quality',async()=>{
  ...
  const fixtureResponse=(await value.app.inject({method:'POST',url:'/api/v1/manufacturing/fixtures',headers:h,payload:{code:'FIX-CI-01',name:'CI Fixture'}}));expect(fixtureResponse.statusCode).toBe(201);
  const cap=(await value.app.inject({method:'POST',url:'/api/v1/manufacturing/capabilities',headers:h,payload:{machineId,operationCode:'OP-10',capabilityName:'CI Operation',minValue:1,maxValue:100,toolingId:tool.json().tool.id,fixtureId:fixtureResponse.json().fixture.id}}));expect(cap.statusCode).toBe(201);
  ...
  const wo=(await value.app.inject({method:'POST',url:'/api/v1/manufacturing/work-orders',headers:h,payload:{workOrderNumber:'CI-WO-001',itemId:itemId,processDetailId:processId,plannedQuantity:10}}));expect(wo.statusCode).toBe(201);
