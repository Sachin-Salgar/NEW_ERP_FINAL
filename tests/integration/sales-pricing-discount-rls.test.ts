...
describe('Sales pricing and discount PostgreSQL boundaries', () => {
  ...
  afterAll(async () => {
    ...
    for (const tenant of [tenantA, tenantB])
      await withTenantContext(pool, 'app.current_tenant_id', tenant, (c) =>
        c.query('DELETE FROM tenants WHERE id=$1', [tenant]),
      );
    await pool.end();
  });
  ...
  runIfDatabase('enforces tenant/org/branch RLS, item resolution, lifecycle, and concurrency', async () => {
    ...
    const suffix = uuidV7();
    await pool.query(
      `INSERT INTO tenants(id,name,subdomain,slug) VALUES($1,'Pricing A',$2,$2),($3,'Pricing B',$4,$4)`,
      [tenantA, `pricing-a-${suffix}`, tenantB, `pricing-b-${suffix}`],
    );
    await withTenantContext(pool, 'app.current_tenant_id', tenantA, async (c) => {
      await c.query(`INSERT INTO organizations(id,tenant_id,code,name) VALUES($1,$2,$3,'Org A'),($4,$2,$5,'Org B')`, [
        orgA,
        tenantA,
        `PA${suffix.slice(0, 6)}`,
        orgB,
      ...
      await c.query(
        `INSERT INTO branches(id,tenant_id,organization_id,code,name,is_default) VALUES($1,$2,$3,$4,'Branch A',true)`,
        [branchA, tenantA, orgA, `PBR${suffix.slice(0, 5)}`],
      );
    });
    await withTenantContext(pool, 'app.current_tenant_id', tenantB, (c) =>
      c.query(`INSERT INTO organizations(id,tenant_id,code,name) VALUES($1,$2,$3,'Org B')`, [
        uuidV7(),
        tenantB,
        `PC${suffix.slice(0, 6)}`,
      ]),
    );
    const pricing = new PostgresPricingRepository(pool);
    const discounts = new PostgresDiscountRepository(pool);
    const list = await pricing.create({
      tenantId: tenantA,
      organizationId: orgA,
      branchId: branchA,
      code: `BRANCH-${suffix.slice(0, 6)}`,
      name: 'Branch prices',
      currency: 'INR',
      effectiveFrom: '2026-01-01',
      actorUserId: actor,
    });
    await pricing.addItem({
      tenantId: tenantA,
      organizationId: orgA,
      priceListId: list.id,
      itemCode: 'ITEM-1',
    ...
    ).resolves.toMatchObject({ status: 'PUBLISHED', versionNumber: 2 });
    await expect(
    ...
    ).resolves.toMatchObject({ price: 125 });
    await expect(
      pricing.transition({
        tenantId: tenantA,
        organizationId: orgA,
    ...
    const rls = await pool!.query(
      `SELECT relrowsecurity,relforcerowsecurity,relname FROM pg_class WHERE relname IN ('sales_price_lists','sales_price_list_items','sales_discount_rules')`,
    );
