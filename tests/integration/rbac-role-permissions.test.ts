...
describe('GET /rbac/roles/:roleId/permissions', () => {
  ...
  runIfDatabase('returns assigned permissions for a role for authorized user and tenant', async () => {
    ...
    const tenantInput = {
      tenant: { name: `RBAC Test ${unique}`, displayName: `RBAC Test ${unique}`, subdomain: `rbac-${unique}`, slug: `rbac-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
  ...
  runIfDatabase('rejects unauthenticated requests with 401', async () => {
    ...
    const unique = `${Date.now()}-${uuidV7()}`;
    const tenantInput = {
      tenant: { name: `RBAC UAT ${unique}`, displayName: `RBAC UAT ${unique}`, subdomain: `rbac-uat-${unique}`, slug: `rbac-uat-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
    ...
    const config = parseAppConfig({
      ...process.env,
      NODE_ENV: 'test', APP_NAME: 'new-erp-final', HOST: '127.0.0.1', PORT: '3001', API_PREFIX: '/api/v1',
      LOG_LEVEL: 'info', DATABASE_URL: databaseUrl!, DATABASE_POOL_MIN: '1', DATABASE_POOL_MAX: '10', JWT_SECRET: '12345678901234567890123456789012', JWT_ISSUER: 'new-erp-final', TENANT_HEADER: 'x-tenant-id', TENANT_CONTEXT_KEY: 'app.current_tenant_id',
    });

    app = await createApplication(config, pool);

    // create role as admin (but we will call GET unauthenticated)
    const adminLogin = await app.inject({ method: 'POST', url: '/api/v1/auth/login', headers: { host: tenantInput.tenant.subdomain, 'x-tenant-id': tenantResult.tenantId }, payload: { identifier: tenantInput.administrator.username, password: tenantInput.administrator.password } });
    expect(adminLogin.statusCode).toBe(200);
    const adminToken = adminLogin.json().accessToken;

    const roleCreate = await app.inject({ method: 'POST', url: '/api/v1/rbac/roles', headers: { authorization: `Bearer ${adminToken}`, 'x-tenant-id': tenantResult.tenantId }, payload: { code: 'viewer', name: 'Viewer' } });
    expect(roleCreate.statusCode).toBe(200);
    const role = roleCreate.json().role;

    const unauth = await app.inject({ method: 'GET', url: `/api/v1/rbac/roles/${role.id}/permissions`, headers: { 'x-tenant-id': tenantResult.tenantId } });
  ...
  runIfDatabase('returns 403 for authenticated user without role.manage', async () => {
    ...
    const unique = `${Date.now()}-${uuidV7()}`;
    const tenantInput = {
      tenant: { name: `RBAC F ${unique}`, displayName: `RBAC F ${unique}`, subdomain: `rbac-f-${unique}`, slug: `rbac-f-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
    ...
    const reg = await app.inject({ method: 'POST', url: '/api/v1/auth/register', headers: { authorization: `Bearer ${adminToken}`, 'x-tenant-id': tenantResult.tenantId }, payload: { username: `user${unique}`, email: `user${unique}@example.com`, password: 'Password456!', roleCode: 'member' } });
    expect(reg.statusCode === 201 || reg.statusCode === 200).toBeTruthy();
    const user = reg.json().user;

    const userLogin = await app.inject({ method: 'POST', url: '/api/v1/auth/login', headers: { host: tenantInput.tenant.subdomain, 'x-tenant-id': tenantResult.tenantId }, payload: { identifier: user.username, password: 'Password456!' } });
    ...
    const roleCreate = await app.inject({ method: 'POST', url: '/api/v1/rbac/roles', headers: { authorization: `Bearer ${adminToken}`, 'x-tenant-id': tenantResult.tenantId }, payload: { code: 'auditor', name: 'Auditor' } });
    expect(roleCreate.statusCode).toBe(200);
    const role = roleCreate.json().role;

    const assignResp = await app.inject({ method: 'POST', url: `/api/v1/rbac/roles/${role.id}/permissions`, headers: { authorization: `Bearer ${adminToken}`, 'x-tenant-id': tenantResult.tenantId }, payload: { permissionKeys: ['permission.read'] } });
    expect(assignResp.statusCode).toBe(200);

    const forbidden = await app.inject({ method: 'GET', url: `/api/v1/rbac/roles/${role.id}/permissions`, headers: { authorization: `Bearer ${userToken}`, 'x-tenant-id': tenantResult.tenantId } });
  ...
  runIfDatabase('enforces tenant isolation: does not leak other tenant permissions', async () => {
    ...
    const unique = `${Date.now()}-${uuidV7()}`;
    const tenantA = {
      tenant: { name: `TA ${unique}`, displayName: `TA ${unique}`, subdomain: `ta-${unique}`, slug: `ta-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
    ...
    const tenantB = {
      tenant: { name: `TB ${unique}`, displayName: `TB ${unique}`, subdomain: `tb-${unique}`, slug: `tb-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
    ...
    const config = parseAppConfig({ ...process.env, NODE_ENV: 'test', APP_NAME: 'new-erp-final', HOST: '127.0.0.1', PORT: '3001', API_PREFIX: '/api/v1', LOG_LEVEL: 'info', DATABASE_URL: databaseUrl!, DATABASE_POOL_MIN: '1', DATABASE_POOL_MAX: '10', JWT_SECRET: '12345678901234567890123456789012', JWT_ISSUER: 'new-erp-final', TENANT_HEADER: 'x-tenant-id', TENANT_CONTEXT_KEY: 'app.current_tenant_id' });
    app = await createApplication(config, pool);

    // admin A login
    const adminALogin = await app.inject({ method: 'POST', url: '/api/v1/auth/login', headers: { host: tenantA.tenant.subdomain, 'x-tenant-id': resA.tenantId }, payload: { identifier: tenantA.administrator.username, password: tenantA.administrator.password } });
    expect(adminALogin.statusCode).toBe(200);
    const tokenA = adminALogin.json().accessToken;

    // admin B login
    const adminBLogin = await app.inject({ method: 'POST', url: '/api/v1/auth/login', headers: { host: tenantB.tenant.subdomain, 'x-tenant-id': resB.tenantId }, payload: { identifier: tenantB.administrator.username, password: tenantB.administrator.password } });
    ...
    const roleAResp = await app.inject({ method: 'POST', url: '/api/v1/rbac/roles', headers: { authorization: `Bearer ${tokenA}`, 'x-tenant-id': resA.tenantId }, payload: { code: 'role-a', name: 'Role A' } });
    expect(roleAResp.statusCode).toBe(200);
    const roleA = roleAResp.json().role;
    await app.inject({ method: 'POST', url: `/api/v1/rbac/roles/${roleA.id}/permissions`, headers: { authorization: `Bearer ${tokenA}`, 'x-tenant-id': resA.tenantId }, payload: { permissionKeys: ['permission.read'] } });

    // create role in tenant B and assign different permission 'role.read'
    const roleBResp = await app.inject({ method: 'POST', url: '/api/v1/rbac/roles', headers: { authorization: `Bearer ${tokenB}`, 'x-tenant-id': resB.tenantId }, payload: { code: 'role-b', name: 'Role B' } });
    expect(roleBResp.statusCode).toBe(200);
    const roleB = roleBResp.json().role;
    await app.inject({ method: 'POST', url: `/api/v1/rbac/roles/${roleB.id}/permissions`, headers: { authorization: `Bearer ${tokenB}`, 'x-tenant-id': resB.tenantId }, payload: { permissionKeys: ['role.read'] } });

    // Now request tenant A role permissions while authenticated as admin A
    const getA = await app.inject({ method: 'GET', url: `/api/v1/rbac/roles/${roleA.id}/permissions`, headers: { authorization: `Bearer ${tokenA}`, 'x-tenant-id': resA.tenantId } });
    ...
    const getBfromA = await app.inject({ method: 'GET', url: `/api/v1/rbac/roles/${roleB.id}/permissions`, headers: { authorization: `Bearer ${tokenA}`, 'x-tenant-id': resA.tenantId } });
  ...
  runIfDatabase('returns empty list for unknown/nonexistent role', async () => {
    ...
    const unique = `${Date.now()}-${uuidV7()}`;
    const tenantInput = {
      tenant: { name: `RBAC NX ${unique}`, displayName: `RBAC NX ${unique}`, subdomain: `rbac-nx-${unique}`, slug: `rbac-nx-${unique}`, timezone: 'UTC', currency: 'USD', locale: 'en_US' },
    ...
    const resp = await app.inject({ method: 'GET', url: `/api/v1/rbac/roles/${unknownId}/permissions`, headers: { authorization: `Bearer ${token}`, 'x-tenant-id': tenantResult.tenantId } });
