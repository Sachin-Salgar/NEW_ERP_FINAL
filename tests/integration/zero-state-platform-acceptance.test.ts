...
describe('fresh zero-state platform acceptance', () => {
  ...
  it('bootstraps, isolates, revokes, audits, and rolls back on a clean database', async () => {
    ...
    const cliEnv = { ...process.env, DATABASE_URL: databaseUrl, PLATFORM_ADMIN_BOOTSTRAP_SECRET: secret, PLATFORM_ADMIN_BOOTSTRAP_PASSWORD: password };
    const tsx = fileURLToPath(new URL('../../node_modules/tsx/dist/cli.mjs', import.meta.url));
    const runBootstrap = (environment: NodeJS.ProcessEnv) => execFileAsync(process.execPath, [tsx, 'scripts/platform-admin.ts', 'bootstrap', email], { env: environment, cwd: process.cwd() });
    ...
    const plaintext = await setupPool.query(`SELECT count(*)::int AS count FROM pg_catalog.pg_tables WHERE schemaname = 'public'`);
    expect(plaintext.rows[0].count).toBeGreaterThan(0);

    appPool = new Pool({ connectionString: databaseUrl });
    app = await createApplication(parseAppConfig({ ...process.env, DATABASE_URL: databaseUrl, NODE_ENV: 'test' }), appPool);
    await app.ready();
    const request = async (method: 'GET' | 'POST' | 'PATCH', url: string, token?: string, payload?: object): Promise<any> =>
      app!.inject({ method, url, headers: token ? { authorization: `Bearer ${token}` } : undefined, payload });

    const platformLogin = await request('POST', '/api/v1/auth/platform-login', undefined, { identifier: email, password });
    ...
    const createTenant = (name: string, user: string) => request('POST', '/api/v1/platform/tenants', platformToken, {
      name, displayName: name, subdomain: `sub-${user}`, slug: `slug-${user}`,
      administrator: { username: user, email: `${user}@example.com`, password },
      organization: { name: `${name} Org` }, branch: { name: `${name} Branch` },
    });
    ...
    const tenantLogin = await request('POST', '/api/v1/auth/login', undefined, { identifier: `tenant-a-${suffix}`, password });
    expect(tenantLogin.statusCode).toBe(200);
    const tenantToken = tenantLogin.json().accessToken as string;
    expect((await request('POST', '/api/v1/auth/context', tenantToken, { contextType: 'tenant', tenantId: tenantB })).statusCode).toBe(403);
    expect((await request('GET', '/api/v1/platform/tenants', tenantToken)).statusCode).toBe(403);
    for (const path of ['/api/v1/platform/members', '/api/v1/platform/roles', '/api/v1/platform/security-policy', '/api/v1/platform/audit']) {
    ...
    const tenantAContext = await request('POST', '/api/v1/auth/context', tenantToken, { contextType: 'tenant', tenantId: tenantA });
    ...
    const customer = await request('POST', '/api/v1/customers', tenantAToken, { name: `Customer ${suffix}`, organizationId });
    expect(customer.statusCode).toBe(201);
    const headerSpoof = await app.inject({ method: 'GET', url: '/api/v1/customers', headers: { authorization: `Bearer ${tenantAToken}`, 'x-tenant-id': tenantB } });
    expect(headerSpoof.statusCode).toBe(200);
    expect(headerSpoof.json().customers.every((entry: { organizationId: string }) => entry.organizationId === organizationId)).toBe(true);
    ...
    const events = await setupPool.query(`SELECT actor_identity_id, actor_platform_membership_id, context_type, tenant_id, target_tenant_id FROM audit_events WHERE context_type = 'platform'`);
    expect(events.rowCount).toBeGreaterThan(0);
    expect(events.rows.some((row) => row.actor_identity_id && row.actor_platform_membership_id && row.context_type === 'platform')).toBe(true);
    const platformMembership = await setupPool.query<{ id: string }>('SELECT id FROM platform_memberships WHERE status = $1 LIMIT 1', ['active']);
    await setupPool.query('UPDATE platform_memberships SET status = $2, revoked_at = now() WHERE id = $1', [platformMembership.rows[0].id, 'revoked']);
    ...
    await expect(uow.runInTransaction(async () => {
      await uow.getClient().query('INSERT INTO security_bootstrap_state (key, value) VALUES ($1, $2)', [`acceptance-${marker}`, '{}']);
      await audit.record({ tenantId: '00000000-0000-0000-0000-000000000000', action: 'acceptance.rollback', resourceType: 'test', resourceId: marker, outcome: 'success' }, { requireTransaction: true });
    })).rejects.toThrow();
    expect((await setupPool.query('SELECT 1 FROM security_bootstrap_state WHERE key = $1', [`acceptance-${marker}`])).rowCount).toBe(0);
  });
});
