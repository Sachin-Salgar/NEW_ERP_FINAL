...
export class PostgresInventoryRepository implements InventoryRepository {
  ...
  async listWarehouses(tenantId: string, organizationId: string, page: number, pageSize: number, search?: string) {
    return this.withTenant(tenantId, organizationId, undefined, async (client) => {
      ...
      const count = await client.query(
        `SELECT count(*)::int AS count FROM inventory_warehouses WHERE ${filters.join(' AND ')}`,
        values,
      );
  ...
  async listStock(
    ...
    return this.withTenant(tenantId, organizationId, undefined, async (client) => {
      ...
      if (itemId) {
        values.push(itemId);
        filters.push(`item_id=$${values.length}`);
      }
      const count = await client.query(
        `SELECT count(*)::int AS count FROM inventory_stock WHERE ${filters.join(' AND ')}`,
        values,
      );
      values.push((page - 1) * pageSize, pageSize);
      const rows = await client.query(
  ...
  async listReservations(input: {
    ...
    return this.withTenant(input.tenantId, input.organizationId, undefined, async (client) => {
      ...
      const count = await client.query(
        `SELECT count(*)::int AS count FROM inventory_reservations WHERE ${filters.join(' AND ')}`,
        values,
      );
  ...
  async listReservationsBySource(input: {
    tenantId: string;
    organizationId: string;
    branchId: string;
    financialYearId: string;
  ...
  async fulfillReservationsBySource(input: {
    tenantId: string;
    organizationId: string;
    branchId: string;
    financialYearId: string;
      ...
      for (const row of rows.rows) {
        results.push(
          await this.fulfillReservation({
            ...
            operationKey: `${input.operationKey}:${row.id}`,
          }),
        );
      }
      return results;
  ...
  private async assertWarehouseAndItem(
    client: PoolClient,
    input: { tenantId: string; organizationId: string; warehouseId: string; itemId: string },
  ) {
  ...
  private async ensureStock(
    client: PoolClient,
    input: { tenantId: string; organizationId: string; warehouseId: string; itemId: string; actorUserId: string },
  ) {
    await client.query(
  ...
  private async lockReservation(
    client: PoolClient,
    input: { tenantId: string; organizationId: string; reservationId: string },
  ) {
    const result = await client.query(
      `SELECT ${reservationColumns} FROM inventory_reservations
  ...
  private async findMovement(
    client: PoolClient,
    input: { tenantId: string; organizationId: string; operationKey: string },
  ) {
    const result = await client.query(
  ...
  private mapStock(row: Record<string, unknown>): InventoryStockRecord {
    return {
      ...row,
  ...
  private mapReservation(row: Record<string, unknown>): InventoryReservationRecord {
    return {
      ...row,
  ...
  private mapMovement(row: Record<string, unknown>): InventoryMovementRecord {
    return {
      ...row,
      quantity: Number(row.quantity),
      createdAt: new Date(String(row.createdAt)),
