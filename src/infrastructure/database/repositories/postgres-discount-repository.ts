...
export class PostgresDiscountRepository implements DiscountRuleRepository {
  ...
  async list(tenantId: string, organizationId: string): Promise<DiscountRuleRecord[]> {
    return withTenantContext(
      ...
      async (client) => {
        ...
        WHERE tenant_id=$1 AND organization_id=$2 ORDER BY code`,
  ...
  async get(tenantId: string, organizationId: string, id: string): Promise<DiscountRuleRecord | null> {
    return withTenantContext(
      ...
      async (client) => {
        const result = await client.query(
          `SELECT ${C} FROM sales_discount_rules WHERE tenant_id=$1 AND organization_id=$2 AND id=$3`,
