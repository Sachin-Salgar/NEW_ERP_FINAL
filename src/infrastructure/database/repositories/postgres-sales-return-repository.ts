...
export class PostgresSalesReturnRepository implements SalesReturnRepository {
  ...
  async create(i: any): Promise<SalesReturnRecord> {
      ...
      async (c) => {
        ...
        const requested = new Map(
          (i.items ?? source.rows.map((x: any) => ({ invoiceItemId: x.id, quantity: Number(x.quantity) }))).map(
            (x: any) => [x.invoiceItemId, x.quantity],
          ),
        );
        if (requested.size === 0) throw new ValidationError('At least one return line is required.');
        for (const item of source.rows) {
          if (!requested.has(item.id)) continue;
          const quantity = Number(requested.get(item.id));
          if (!Number.isFinite(quantity) || quantity <= 0 || quantity > Number(item.quantity))
            throw new ValidationError('Return quantity must be positive and no greater than the invoiced quantity.');
          if (!item.itemId)
            throw new ValidationError('Every new Inventory-backed return line requires an Item Master item.');
        }
        if ([...requested.keys()].some((id) => !source.rows.some((x: any) => x.id === id)))
        ...
        const r = await c.query(
          ...
          [
            ...
            i.invoiceId,
            inv.rows[0].deliveryId,
            inv.rows[0].warehouseId,
            inv.rows[0].customerId,
            i.idempotencyKey,
            i.notes,
            i.actorUserId,
          ],
  ...
  async list(t: string, q: any) {
    return withTenantContext(
      ...
      async (c) => {
        ...
        const count = await c.query(`SELECT count(*)::int count FROM sales_returns WHERE ${f.join(' AND ')}`, v);
        v.push((q.page - 1) * q.pageSize, q.pageSize);
        const d = q.order === 'desc' ? 'DESC' : 'ASC';
        const rows = await c.query(
          `SELECT ${C} FROM sales_returns WHERE ${f.join(' AND ')} ORDER BY return_number ${d},id ${d} OFFSET $${v.length - 1} LIMIT $${v.length}`,
          v,
        );
        return {
  ...
  private async map(c: any, r: any): Promise<SalesReturnRecord> {
    const x = await c.query(
      `SELECT id,line_number AS "lineNumber",invoice_item_id AS "invoiceItemId",item_id AS "itemId",description,quantity,unit_price AS "unitPrice",unit_of_measure AS "unitOfMeasure" FROM sales_return_items WHERE return_id=$1 AND tenant_id=$2 ORDER BY line_number`,
      [r.id, r.tenantId],
    );
    return {
      ...r,
