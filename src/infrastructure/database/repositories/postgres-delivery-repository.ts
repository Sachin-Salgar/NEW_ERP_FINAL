...
export class PostgresDeliveryRepository implements DeliveryRepository {
  ...
  async list(t: string, q: any) {
    return withTenantContext(
      ...
      async (c) => {
        ...
        const count = await c.query(`SELECT count(*)::int count FROM sales_deliveries WHERE ${f.join(' AND ')}`, v);
        v.push((q.page - 1) * q.pageSize, q.pageSize);
        const d = q.order === 'desc' ? 'DESC' : 'ASC';
        const rows = await c.query(
          `SELECT ${C} FROM sales_deliveries WHERE ${f.join(' AND ')} ORDER BY delivery_number ${d},id ${d} OFFSET $${v.length - 1} LIMIT $${v.length}`,
          v,
        );
        return {
  ...
  private async map(c: any, r: any): Promise<DeliveryRecord> {
    const x = await c.query(
      `SELECT id,line_number AS "lineNumber",order_item_id AS "orderItemId",item_id AS "itemId",reservation_id AS "reservationId",description,quantity,unit_of_measure AS "unitOfMeasure" FROM sales_delivery_items WHERE delivery_id=$1 AND tenant_id=$2 ORDER BY line_number`,
      [r.id, r.tenantId],
    );
    return {
      ...r,
      items: x.rows.map((a: any) => ({ ...a, quantity: Number(a.quantity) })),
