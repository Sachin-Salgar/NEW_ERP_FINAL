...
const C = `id,tenant_id AS "tenantId",organization_id AS "organizationId",branch_id AS "branchId",financial_year_id AS "financialYearId",invoice_number AS "invoiceNumber",sales_order_id AS "salesOrderId",delivery_id AS "deliveryId",customer_id AS "customerId",status,idempotency_key AS "idempotencyKey",finance_status AS "financeStatus",tax_status AS "taxStatus",finance_reference AS "financeReference",tax_reference AS "taxReference",notes,created_at AS "createdAt",created_by AS "createdBy",updated_at AS "updatedAt",updated_by AS "updatedBy",version_number AS "versionNumber"`;
export class PostgresInvoiceRepository implements InvoiceRepository {
  ...
  private async map(c:any,r:any):Promise<InvoiceRecord>{const x=await c.query(`SELECT id,line_number AS "lineNumber",delivery_item_id AS "deliveryItemId",description,quantity,unit_price AS "unitPrice",unit_of_measure AS "unitOfMeasure",line_total AS "lineTotal" FROM sales_invoice_items WHERE invoice_id=$1 AND tenant_id=$2 ORDER BY line_number`,[r.id,r.tenantId]);return {...r,items:x.rows.map((a:any)=>({...a,quantity:Number(a.quantity),unitPrice:Number(a.unitPrice),lineTotal:Number(a.lineTotal)})),createdAt:new Date(r.createdAt),updatedAt:r.updatedAt?new Date(r.updatedAt):null};}
}
