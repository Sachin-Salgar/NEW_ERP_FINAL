...
-- It maps login identifiers to tenant user accounts so the backend can discover the tenant
...
COMMENT ON COLUMN auth_login_identifiers.tenant_id IS 'Candidate tenant discovered from the authenticated login identity. Membership and tenant authorization are validated by the application before a tenant session is created.';
