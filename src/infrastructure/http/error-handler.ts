...
export function buildErrorHandler(config: AppConfig) {
  return (error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
    ...
    const exposedMessage =
      appError?.expose === false || statusCode >= 500
    ...
    if (acceptsProblemJson(request)) {
      const type = isValidationError ? 'https://httpstatuses.com/400' : `https://httpstatuses.com/${statusCode}`;
      const problem = {
        type,
        title: isValidationError
          ...
          : statusCode >= 500
            ? 'Internal Server Error'
            : (appError?.code ?? error.code ?? 'Error'),
        status: statusCode,
        detail: exposedMessage,
        instance: request.url,
        errors:
          validationDetails ?? (appError?.details && Array.isArray(appError.details) ? appError.details : undefined),
      };
    ...
    const body = {
      error: {
        ...
        details:
          validationDetails ?? (config.NODE_ENV === 'development' && statusCode < 500 ? appError?.details : undefined),
      },
    };
