...
export class TaxService {
  ...
  async create(
    c: TaxContext,
    input: { code: string; name: string; rate: number; effectiveFrom: string; effectiveTo?: string | null },
  ...
  async update(
    c: TaxContext,
    id: string,
    input: { name: string; rate: number; effectiveTo?: string | null; expectedVersion: number },
  ) {
    await this.authorize(c, 'tax.configuration.update');
    this.id(id);
    if (!input.name?.trim() || !Number.isFinite(input.rate) || input.rate < 0 || input.rate > 100)
      throw new ValidationError('Name and rate between 0 and 100 are required.');
  ...
  async transition(c: TaxContext, id: string, status: TaxStatus, expectedVersion: number) {
    await this.authorize(c, status === 'ACTIVE' ? 'tax.configuration.activate' : 'tax.configuration.deactivate');
    this.id(id);
  ...
  async calculate(c: TaxContext, input: { amount: number; asOf: string }): Promise<TaxResolution> {
    ...
    if (!Number.isFinite(input.amount) || input.amount < 0)
      throw new ValidationError('Taxable amount must be non-negative.');
    ...
    return {
      ...
      code: rule.code,
      rate: rule.rate,
      taxAmount: Number(((input.amount * rule.rate) / 100).toFixed(4)),
  ...
  private id(value: string, label = 'Tax rule ID') {
