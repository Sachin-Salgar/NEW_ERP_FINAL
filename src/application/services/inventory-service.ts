...
export class InventoryService {
  ...
  async createWarehouse(context: InventoryContext, input: { code: string; name: string }) {
    ...
    if (!code || code.length > 100 || !name || name.length > 255)
  ...
  async updateWarehouse(
    ...
    id: string,
    input: { name: string; status: WarehouseStatus; expectedVersion: number },
  ) {
    await this.authorize(context, INVENTORY_PERMISSIONS.warehouseUpdate);
    this.id(id, 'Warehouse ID');
    if (!input.name.trim() || !Number.isInteger(input.expectedVersion) || input.expectedVersion < 1)
      throw new ValidationError('Valid warehouse name and expected version are required.');
    const warehouse = await this.repository.updateWarehouse({
      ...context,
      warehouseId: id,
  ...
  async listStock(
    context: InventoryContext,
    input: { page?: number; pageSize?: number; warehouseId?: string; itemId?: string } = {},
  ) {
  ...
  async receive(
    ...
    await this.authorize(context, INVENTORY_PERMISSIONS.stockReceive);
    this.validateOperation(input);
    return this.tx.runInTransaction(async () =>
      this.repository.receiveStock({ ...context, ...input, actorUserId: context.userId }),
    );
  }
  async reserve(context: InventoryContext, input: InventoryReservationRequest) {
    await this.authorize(context, INVENTORY_PERMISSIONS.reservationCreate);
  ...
  async fulfillReservationsBySource(
    context: InventoryContext,
    sourceType: string,
    sourceId: string,
    operationKey: string,
    ...
    return this.tx.runInTransaction(async () =>
      this.repository.fulfillReservationsBySource({
        ...context,
        sourceType,
        sourceId,
        operationKey,
  ...
  async listReservations(
    context: InventoryContext,
    input: { page?: number; pageSize?: number; status?: ReservationStatus } = {},
  ) {
    await this.authorize(context, INVENTORY_PERMISSIONS.reservationRead);
    const page = input.page ?? 1;
    const pageSize = input.pageSize ?? 20;
  ...
  async release(context: InventoryContext, id: string, operationKey: string) {
    await this.authorize(context, INVENTORY_PERMISSIONS.reservationRelease);
    this.id(id, 'Reservation ID');
    this.key(operationKey);
    return this.tx.runInTransaction(async () =>
      this.repository.releaseReservation({ ...context, reservationId: id, operationKey, actorUserId: context.userId }),
    );
  }
  async fulfill(context: InventoryContext, id: string, operationKey: string) {
    ...
    this.key(operationKey);
    return this.tx.runInTransaction(async () =>
      this.repository.fulfillReservation({ ...context, reservationId: id, operationKey, actorUserId: context.userId }),
    );
  }
  async returnStock(
    context: InventoryContext,
    input: Omit<InventoryReservationRequest, 'idempotencyKey'> & { idempotencyKey: string },
  ) {
    await this.authorize(context, INVENTORY_PERMISSIONS.stockReturn);
    this.validateReservation(input);
    return this.tx.runInTransaction(async () =>
  ...
  private validateContext(c: InventoryContext) {
  ...
  private validateReservation(input: {
    ...
    this.id(input.itemId, 'Item ID');
    this.id(input.sourceId, 'Source ID');
  ...
  private validateOperation(input: {
    warehouseId: string;
    itemId: string;
    quantity: number;
    sourceType: string;
  ...
  private async record(c: InventoryContext, action: string, resourceId: string, resourceType: string) {
    await this.audit.record(
      { tenantId: c.tenantId, actorUserId: c.userId, action, resourceType, resourceId, outcome: 'success' },
