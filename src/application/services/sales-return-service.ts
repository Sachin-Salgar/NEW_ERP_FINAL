...
import {
  SALES_RETURN_PERMISSIONS,
  type SalesReturnPermission,
...
export interface SalesReturnTransactionRunner {
  runInTransaction<T>(callback: () => Promise<T>): Promise<T>;
}
const transitions: Record<SalesReturnStatus, SalesReturnStatus[]> = {
  REQUESTED: ['INSPECTED', 'CANCELLED'],
...
export class SalesReturnService {
  ...
  async create(
    ...
    await this.authorize(context, SALES_RETURN_PERMISSIONS.create);
    this.id(input.invoiceId, 'Invoice ID');
  ...
  async get(context: SalesReturnContext, id: string) {
    ...
    if (!value) throw new NotFoundError('Sales Return not found.');
    return value;
  }
  async list(
    context: SalesReturnContext,
  ...
  async update(context: SalesReturnContext, id: string, input: { notes?: string | null; expectedVersion: number }) {
    await this.authorize(context, SALES_RETURN_PERMISSIONS.update);
    this.id(id, 'Return ID');
    ...
    if (!value) throw new ValidationError('Requested return not found or version conflict.');
    return value;
  }
  async transition(context: SalesReturnContext, id: string, status: SalesReturnStatus, expectedVersion: number) {
    ...
    return this.tx.runInTransaction(async () => {
      ...
      if (!transitions[current.status].includes(status))
        throw new ValidationError(`Sales Return cannot transition from ${current.status} to ${status}.`);
      if (status === 'PROCESSED') {
        if (!this.inventory) throw new ValidationError('Inventory return provider is not configured.');
        if (!current.warehouseId || current.items.some((item) => !item.itemId))
          throw new ValidationError('Return warehouse and Item Master references are required.');
        for (const item of current.items)
          await this.inventory.returnStock(context, {
            warehouseId: current.warehouseId,
            itemId: item.itemId!,
            quantity: item.quantity,
            sourceType: 'SALES_RETURN',
            sourceId: current.id,
            idempotencyKey: `sales-return:${current.id}:${item.invoiceItemId}`,
          });
        const value = await this.repository.process({
          ...context,
          returnId: id,
          expectedVersion,
          actorUserId: context.userId,
      ...
      await this.audit.record(
        {
          tenantId: context.tenantId,
          actorUserId: context.userId,
          action: `sales_return.${status.toLowerCase()}`,
          resourceType: 'sales_return',
          resourceId: id,
          outcome: 'success',
        },
  ...
  private async authorize(c: SalesReturnContext, p: SalesReturnPermission) {
