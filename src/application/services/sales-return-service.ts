...
export class SalesReturnService {
  constructor(private readonly repository: SalesReturnRepository, private readonly auth: Pick<AuthorizationService, 'hasPermission'>, private readonly modules: Pick<ModuleAccessService, 'isModuleEnabled'>, private readonly audit: AuditLogger, private readonly tx: SalesReturnTransactionRunner, private readonly inventory?: InventoryDependencyPort) {}
  async create(context: SalesReturnContext, input: { invoiceId: string; idempotencyKey: string; notes?: string | null; items?: Array<{ invoiceItemId: string; quantity: number }> }): Promise<SalesReturnRecord> {
    ...
    const allowReplay = await this.auth.hasPermission(context.tenantId, context.userId, SALES_RETURN_PERMISSIONS.read);
    return this.tx.runInTransaction(async () => { const value = await this.repository.create({ ...context, invoiceId: input.invoiceId, idempotencyKey: key, notes: input.notes ?? null, items: input.items, actorUserId: context.userId, allowReplay }); await this.audit.record({ tenantId: context.tenantId, actorUserId: context.userId, action: 'sales_return.created', resourceType: 'sales_return', resourceId: value.id, outcome: 'success' }, { requireTransaction: true }); return value; });
  }
  ...
  async transition(context: SalesReturnContext, id: string, status: SalesReturnStatus, expectedVersion: number) { const action = status === 'INSPECTED' ? 'inspect' : status === 'APPROVED' ? 'approve' : status === 'REJECTED' ? 'reject' : status === 'PROCESSED' ? 'process' : status === 'CLOSED' ? 'close' : 'cancel'; await this.authorize(context, SALES_RETURN_PERMISSIONS[action]); this.id(id, 'Return ID'); this.version(expectedVersion); return this.tx.runInTransaction(async () => { const current = await this.get(context, id); if (status === 'PROCESSED' && current.status === 'PROCESSED' && current.inventoryStatus === 'COMPLETED') return current; if (!transitions[current.status].includes(status)) throw new ValidationError(`Sales Return cannot transition from ${current.status} to ${status}.`); if (status === 'PROCESSED') { if (!this.inventory) throw new ValidationError('Inventory return provider is not configured.'); if (!current.warehouseId || current.items.some((item) => !item.itemId)) throw new ValidationError('Return warehouse and Item Master references are required.'); for (const item of current.items) await this.inventory.returnStock(context, { warehouseId: current.warehouseId, itemId: item.itemId!, quantity: item.quantity, sourceType: 'SALES_RETURN', sourceId: current.id, idempotencyKey: `sales-return:${current.id}:${item.invoiceItemId}` }); const value = await this.repository.process({ ...context, returnId: id, expectedVersion, actorUserId: context.userId }); if (!value) throw new ValidationError('Sales Return not found or version conflict.'); await this.audit.record({ tenantId: context.tenantId, actorUserId: context.userId, action: 'sales_return.processed', resourceType: 'sales_return', resourceId: id, outcome: 'success' }, { requireTransaction: true }); return value; } const value = await this.repository.transition({ ...context, returnId: id, status, expectedVersion, actorUserId: context.userId }); if (!value) throw new ValidationError('Sales Return not found or version conflict.'); await this.audit.record({ tenantId: context.tenantId, actorUserId: context.userId, action: `sales_return.${status.toLowerCase()}`, resourceType: 'sales_return', resourceId: id, outcome: 'success' }, { requireTransaction: true }); return value; }); }
