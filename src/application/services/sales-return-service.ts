...
export class SalesReturnService {
  ...
  async transition(context: SalesReturnContext, id: string, status: SalesReturnStatus, expectedVersion: number) { const action = status === 'INSPECTED' ? 'inspect' : status === 'APPROVED' ? 'approve' : status === 'REJECTED' ? 'reject' : status === 'PROCESSED' ? 'process' : status === 'CLOSED' ? 'close' : 'cancel'; await this.authorize(context, SALES_RETURN_PERMISSIONS[action]); this.id(id, 'Return ID'); this.version(expectedVersion); return this.tx.runInTransaction(async () => { const current = await this.get(context, id); if (!transitions[current.status].includes(status)) throw new ValidationError(`Sales Return cannot transition from ${current.status} to ${status}.`); if (status === 'PROCESSED') { if (!this.inventory || !this.repository.updateInventoryStatus) throw new ValidationError('Inventory return provider is not configured.'); if (current.inventoryStatus === 'COMPLETED') return current; if (!current.warehouseId || current.items.some((item) => !item.itemId)) throw new ValidationError('Return warehouse and Item Master references are required.'); for (const item of current.items) { await this.inventory.returnStock(context, { warehouseId: current.warehouseId, itemId: item.itemId!, quantity: item.quantity, sourceType: 'SALES_RETURN', sourceId: current.id, idempotencyKey: `sales-return:${current.id}:${item.invoiceItemId}` }); } const integrated = await this.repository.updateInventoryStatus({ ...context, returnId: id, status: 'COMPLETED', actorUserId: context.userId }); if (!integrated) throw new ValidationError('Return Inventory status could not be persisted.'); } const value = await this.repository.transition({ ...context, returnId: id, status, expectedVersion, actorUserId: context.userId }); if (!value) throw new ValidationError('Sales Return not found or version conflict.'); await this.audit.record({ tenantId: context.tenantId, actorUserId: context.userId, action: `sales_return.${status.toLowerCase()}`, resourceType: 'sales_return', resourceId: id, outcome: 'success' }, { requireTransaction: true }); return value; }); }
