...
export interface InvoiceTransactionRunner {
...
export class InvoiceService {
  ...
  async create(
    ...
    input: { deliveryId: string; idempotencyKey: string; notes?: string | null },
    ...
    const allowReplay = await this.authorizationService.hasPermission(
      context.tenantId,
      context.userId,
  ...
  async transition(context: InvoiceContext, id: string, status: InvoiceStatus, expectedVersion: number) {
    ...
    return this.transactionRunner.runInTransaction(async () => {
      ...
      if (!transitions[current.status].includes(status))
        throw new ValidationError(`Invoice cannot transition from ${current.status} to ${status}.`);
      if (status === 'ISSUED') {
        if (!this.tax || !this.repository.updateTaxSnapshot)
          throw new ValidationError('Tax provider is not configured.');
        const taxableAmount = current.items.reduce((total, item) => total + item.lineTotal, 0);
        const result = await this.tax.calculate(
          { ...context, actorUserId: context.userId, correlationId: id, idempotencyKey: `invoice-tax:${id}` },
          'INVOICE',
          id,
          taxableAmount,
        );
        if (
          ...
          result.taxableAmount === undefined ||
          result.taxAmount === undefined
        )
          throw new ValidationError('Authoritative tax calculation failed.');
        const snapshotted = await this.repository.updateTaxSnapshot({
          ...context,
          invoiceId: id,
          taxReference: result.reference,
          taxRate: result.rate,
        ...
        const posting = await this.finance.submitSalesDocument(
          { ...context, actorUserId: context.userId, correlationId: id, idempotencyKey: `invoice-finance:${id}` },
          'INVOICE',
          id,
          result.taxableAmount + result.taxAmount,
        );
      ...
      const invoice = await this.repository.transition({
        ...context,
        invoiceId: id,
        status,
        expectedVersion: status === 'ISSUED' ? expectedVersion + 2 : expectedVersion,
        actorUserId: context.userId,
      });
  ...
  private validateId(value: string, label: string) {
    if (!value || !isUuid(value)) throw new ValidationError(`${label} must be a valid UUID.`);
  }
  private validateVersion(value: number) {
