...
export class PricingService {
  ...
  async update(
    c: PricingContext,
    id: string,
    i: { name: string; effectiveTo?: string | null; expectedVersion: number },
  ) {
    await this.authorize(c, 'sales.pricing.update');
    this.id(id);
    ...
    if (!x) throw new ValidationError('Draft price list not found or version conflict.');
    return x;
  }
  async addItem(
    c: PricingContext,
    id: string,
    i: { itemCode: string; unitOfMeasure: string; price: number; effectiveFrom: string; effectiveTo?: string | null },
  ) {
    await this.authorize(c, 'sales.pricing.update');
    this.id(id);
    if (
      ...
      throw new ValidationError('Item, UOM, non-negative price, and a valid effective period are required.');
  ...
  async transition(c: PricingContext, id: string, status: PriceListStatus, expectedVersion: number) {
    ...
    this.id(id);
    const x = await this.repository.transition({ ...c, id, status, expectedVersion, actorUserId: c.userId });
    if (!x) throw new ValidationError('Price list transition failed or version conflict.');
    await this.audit.record({
      tenantId: c.tenantId,
      actorUserId: c.userId,
      action: `pricing.${status.toLowerCase()}`,
      resourceType: 'sales_price_list',
      resourceId: id,
      outcome: 'success',
    });
  ...
  private async authorize(c: PricingContext, p: string) {
  ...
  private id(v: string, l = 'Price List ID') {
