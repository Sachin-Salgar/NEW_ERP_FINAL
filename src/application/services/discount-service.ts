...
export class DiscountService {
  constructor(
    private readonly r: DiscountRuleRepository,
    private readonly a: Pick<AuthorizationService, 'hasPermission'>,
    private readonly m: Pick<ModuleAccessService, 'isModuleEnabled'>,
    private readonly audit: AuditLogger,
  ) {}
  async create(
    c: DiscountContext,
    i: { code: string; name: string; percentage: number; effectiveFrom: string; effectiveTo?: string | null },
  ): Promise<DiscountRuleRecord> {
    await this.auth(c, 'sales.discount.create');
    if (!i.code?.trim() || !i.name?.trim() || !Number.isFinite(i.percentage) || i.percentage <= 0 || i.percentage > 100)
      throw new ValidationError('Code, name, and percentage between 0 and 100 are required.');
    const x = await this.r.create({ ...c, ...i, effectiveTo: i.effectiveTo ?? null, actorUserId: c.userId });
    await this.audit.record({
      tenantId: c.tenantId,
      actorUserId: c.userId,
      action: 'discount.created',
      resourceType: 'sales_discount_rule',
      resourceId: x.id,
      outcome: 'success',
    });
    return x;
  ...
  async update(
    ...
    await this.auth(c, 'sales.discount.update');
    this.id(id);
    if (
      !i.name?.trim() ||
      !Number.isFinite(i.percentage) ||
      i.percentage <= 0 ||
      i.percentage > 100 ||
      !i.effectiveFrom ||
      (i.effectiveTo !== undefined && i.effectiveTo !== null && i.effectiveTo < i.effectiveFrom)
    )
      throw new ValidationError('Name, percentage, and a valid effective period are required.');
    const x = await this.r.update({
      ...c,
      id,
      name: i.name.trim(),
    ...
    if (!x) throw new ValidationError('Draft discount rule not found or version conflict.');
    await this.audit.record({
      tenantId: c.tenantId,
      actorUserId: c.userId,
  ...
  async transition(c: DiscountContext, id: string, status: DiscountStatus, expectedVersion: number) {
    ...
    this.id(id);
    const x = await this.r.transition({ ...c, id, status, expectedVersion, actorUserId: c.userId });
    if (!x) throw new ValidationError('Discount transition failed or version conflict.');
    await this.audit.record({
      tenantId: c.tenantId,
      actorUserId: c.userId,
  ...
  private id(v: string) {
    if (!isUuid(v)) throw new ValidationError('Context identifier must be a valid UUID.');
  }
}
