...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request: any) => ({
      success: true,
      priceList: await fastify.pricingService.get(context(request), request.params.id),
    }),
  );
  fastify.post(
    ...
    async (request: any, reply) => {
      ...
      return {
        success: true,
        item: await fastify.pricingService.addItem(context(request), request.params.id, request.body),
      };
    },
  );
  fastify.get(
    '/sales/price-lists/resolve',
    {
      preHandler: [requireAuth, requirePermission('sales.pricing.read')],
    },
    async (request: any) => {
  ...
  fastify.patch(
    ...
    async (request: any) => ({
      success: true,
      priceList: await fastify.pricingService.update(context(request), request.params.id, request.body),
    }),
  );
  for (const status of ['publish', 'archive'] as const) {
    fastify.post(
      `/sales/price-lists/:id/${status}`,
      {
