...
async function audit(
  ...
  request: { identityId?: string; platformMembershipId?: string },
  action: string,
  resourceType: string,
...
const platformAdministrationRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.patch<{
    ...
    Body: { status?: 'active' | 'suspended' | 'revoked'; roleIds?: string[] };
  }>(
    '/platform/members/:membershipId',
    { preHandler: requirePlatformContext('platform.members.manage') },
    async (request) => {
      ...
      try {
        ...
        if (request.body.status)
          await client.query(
            `UPDATE platform_memberships SET status = $2, updated_at = now(), revoked_at = CASE WHEN $2 = 'revoked' THEN now() ELSE revoked_at END WHERE id = $1`,
            [membershipId, request.body.status],
          );
        ...
        await client.query(
          "UPDATE user_sessions SET is_active = false WHERE platform_membership_id = $1 AND $2 IN ('suspended', 'revoked')",
          [membershipId, request.body.status ?? 'active'],
        );
  ...
  fastify.post<{ Body: z.infer<typeof roleBody> }>(
    '/platform/roles',
  ...
  fastify.patch<{ Params: { roleId: string }; Body: Partial<z.infer<typeof roleBody>> & { isDeleted?: boolean } }>(
  ...
  fastify.put<{ Params: { roleId: string }; Body: { permissionIds: string[] } }>(
    '/platform/roles/:roleId/permissions',
    { preHandler: requirePlatformContext('platform.permissions.manage') },
    async (request) => {
      const roleId = uuid.parse(request.params.roleId);
      ...
      try {
        ...
        const role = await client.query(
          'SELECT id FROM platform_roles WHERE id = $1 AND is_system = false AND is_deleted = false FOR UPDATE',
          [roleId],
        );
        ...
        for (const permissionId of permissionIds)
          await client.query(
            'INSERT INTO platform_role_permissions (platform_role_id, platform_permission_id) VALUES ($1, $2)',
            [roleId, permissionId],
          );
  ...
  fastify.patch<{ Body: z.infer<typeof policyBody> }>(
    '/platform/security-policy',
    { preHandler: requirePlatformContext('platform.security.manage') },
    async (request) => {
      ...
      try {
        ...
        const result = await client.query(
         ...
         WHERE id = true RETURNING mfa_required AS "mfaRequired", session_lifetime_minutes AS "sessionLifetimeMinutes", max_failed_login_attempts AS "maxFailedLoginAttempts", lockout_minutes AS "lockoutMinutes"`,
          [
            body.mfaRequired ?? null,
            body.sessionLifetimeMinutes ?? null,
            body.maxFailedLoginAttempts ?? null,
