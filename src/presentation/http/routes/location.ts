...
const locationRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Params: LocationIdParams }>(
    ...
    { preHandler: [requireAuth, requirePermission('organization.location.activate')] },
    async (request) => {
      if (!request.tenantId || !request.user?.organizationId)
        throw new ValidationError('Authenticated organization context is required.');
      const activated = await request.server.locationService.activateLocation(
        request.tenantId,
  ...
  fastify.delete<{ Params: LocationIdParams }>(
    '/locations/:id',
    { preHandler: [requireAuth, requirePermission('organization.location.delete')] },
    async (request) => {
      if (!request.tenantId || !request.user?.organizationId)
        throw new ValidationError('Authenticated organization context is required.');
      const deleted = await request.server.locationService.deleteLocation(
        request.tenantId,
        request.user.organizationId,
        request.params.id.trim(),
      );
