...
const locationRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{ Params: LocationIdParams }>('/locations/:id', { preHandler: [requireAuth, requirePermission('organization.read')] }, async (request) => {
    if (!request.tenantId || !request.user) throw new ValidationError('Authenticated tenant context is required.');
    const organizationId = request.user.organizationId ?? null;
    if (!organizationId) throw new ValidationError('An active organization is required before resolving locations.');
  ...
  fastify.post<{ Params: LocationIdParams }>('/locations/:id/select', { preHandler: [requireAuth, requirePermission('organization.read')] }, async (request, reply) => {
    if (!request.tenantId || !request.user) throw new ValidationError('Authenticated tenant context is required.');
    const organizationId = request.user.organizationId ?? null;
    ...
    return {
      success: true,
      user: { id: result.user.id, tenantId: result.user.tenantId, organizationId: result.user.organizationId ?? organizationId, activeLocationId: result.user.activeLocationId ?? location.id, defaultLocationId: result.user.defaultLocationId ?? null, defaultBranchId: result.user.defaultBranchId ?? null, username: result.user.username, email: result.user.email, status: result.user.status },
  ...
  fastify.post<{ Body: CreateLocationBody }>('/locations', { preHandler: [requireAuth, requirePermission('organization.manage')] }, async (request, reply) => {
    if (!request.tenantId || !request.user) throw new ValidationError('Authenticated tenant context is required.');
    ...
    const location = await request.server.locationService.createLocation(request.tenantId, organizationId, {
      ...
      status: body.status ?? 'active', isDefault: body.isDefault ?? false,
      ...
      postalCode: body.postalCode ?? null, timezone: body.timezone ?? 'UTC',
    });
    reply.code(201);
    return { success: true, location };
  });

  fastify.patch<{ Params: LocationIdParams; Body: UpdateLocationBody }>('/locations/:id', { preHandler: [requireAuth, requirePermission('organization.manage')] }, async (request) => {
    if (!request.tenantId || !request.user) throw new ValidationError('Authenticated tenant context is required.');
    const locationId = request.params.id.trim();
    const body = request.body;
  ...
  fastify.post<{ Params: LocationIdParams }>('/locations/:id/deactivate', { preHandler: [requireAuth, requirePermission('organization.manage')] }, async (request) => {
    if (!request.tenantId || !request.user) throw new ValidationError('Authenticated tenant context is required.');
    const locationId = request.params.id.trim();
    const currentLocation = await request.server.locationService.getAccessibleLocationByIdForUser(request.tenantId, request.user.id, locationId, request.user.organizationId ?? null);
    if (!currentLocation) throw new NotFoundError('Location not found or access denied.');
