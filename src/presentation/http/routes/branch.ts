...
const branchRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/branches', { preHandler: [requireAuth, requirePermission('organization.read')] }, async (request) => {
    ...
    const branches = await request.server.branchService.listAccessibleBranchesForUser(request.tenantId, request.user.id, organizationId);
  ...
  fastify.post('/branches/:id/select', { preHandler: [requireAuth, requirePermission('organization.read')] }, async (request, reply) => {
    ...
    const branchId = asString((request.params as { id?: string }).id) ?? '';
    ...
    return {
      ...
      user: {
        ...
        activeLocationId: result.user.activeLocationId ?? request.user.activeLocationId ?? request.user.defaultLocationId ?? null,
      ...
      session: {
        ...
        locationId: result.session.locationId ?? request.user.activeLocationId ?? request.user.defaultLocationId ?? null,
