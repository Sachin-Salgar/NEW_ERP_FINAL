...
function response(item: ItemRecord) {
  return {
    ...
    salesEligible: item.salesEligible,
    status: item.status,
...
const itemMasterRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    '/inventory/items',
    { preHandler: [requireAuth, requirePermission('inventory.item.read')] },
    async (request) => {
      const ctx = context(request);
      ...
      const result = await request.server.itemMasterService.list(ctx, {
        page: query.page,
  ...
  fastify.get<{ Params: ItemParams }>(
    ...
    async (request) => {
      return {
        success: true,
        item: response(
          await request.server.itemMasterService.get(context(request), requestParam(request.params, 'id') ?? ''),
        ),
      };
    },
  );

  fastify.patch<{ Params: ItemParams; Body: UpdateItemBody }>(
    '/inventory/items/:id',
    { preHandler: [requireAuth, requirePermission('inventory.item.update')] },
    async (request) => {
      return {
        success: true,
        item: response(
          await request.server.itemMasterService.update(
            context(request),
  ...
  fastify.delete<{ Params: ItemParams; Body: { expectedVersion: number } }>(
    '/inventory/items/:id',
    { preHandler: [requireAuth, requirePermission('inventory.item.delete')] },
    async (request) => {
      return {
        success: true,
        item: response(
          await request.server.itemMasterService.softDelete(
            context(request),
            requestParam(request.params, 'id') ?? '',
