...
const ctx=(r:FastifyRequest)=>{if(!r.tenantId||!r.user?.branchId||!r.user?.financialYearId)throw new ValidationError('Authenticated tenant, branch, and financial-year context is required.');return{tenantId:r.tenantId,branchId:r.user.branchId,financialYearId:r.user.financialYearId,userId:r.user.id};};
