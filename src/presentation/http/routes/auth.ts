...
const authRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{ Body: z.infer<typeof authSchemas.registerRequest> }>('/auth/register', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.post<{ Body: z.infer<typeof authSchemas.loginRequest> }>('/auth/login', {
    schema: {
      tags: ['Authentication'],
      summary: 'Authenticate user and create session',
      description: 'Authenticate with username/email and password. Returns access token, refresh token, and user context.',
  ...
  fastify.post<{ Body: z.infer<typeof authSchemas.refreshRequest> }>('/auth/refresh', {
    schema: {
      tags: ['Authentication'],
      ...
      response: {
        200: toJsonSchema(z.object({
          ...
          organizations: z.array(z.object({
            id: z.string().uuid(), tenantId: z.string().uuid(), code: z.string(), name: z.string(), status: z.string(), isDefault: z.boolean(),
          })),
  ...
  fastify.post<{ Body: z.infer<typeof authSchemas.orgSelectRequest> }>('/auth/organizations/select', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.post<{ Body: z.infer<typeof authSchemas.contextSelectRequest> }>('/auth/context/select', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.post<{ Params: ModuleCodeParams }>('/auth/modules/:code/enable', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.post<{ Params: ModuleCodeParams }>('/auth/modules/:code/disable', {
    schema: {
      tags: ['Authentication'],
  ...
  fastify.get('/auth/protected', {
    ...
    if (!request.user || !request.tenantId || !request.sessionId) throw new UnauthorizedError('Authentication required.');
    return { success: true, user: sanitizeUser(request.user), tenantId: request.tenantId, sessionId: request.sessionId };
  });
};
