...
const codeSchema = z.object({ token: z.string().trim().regex(/^\d{6}$/, 'MFA code must contain exactly 6 digits') });
...
const mfaRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.post<{ Body: z.infer<typeof beginSchema> }>('/auth/mfa/enroll', {
    schema: {
      ...
      response: {
        200: toJsonSchema(z.object({ success: z.literal(true), secret: z.string(), otpauthUri: z.string(), expiresAt: z.string().datetime() })),
  ...
  fastify.post<{ Body: z.infer<typeof codeSchema> }>('/auth/mfa/enroll/confirm', {
    ...
    try {
      const recoveryCodes = await request.server.mfaService.confirmEnrollment(request.tenantId, request.user.id, body.token);
