...
const coreEnterpriseRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      return { success: true, organizations: await request.server.coreEnterpriseService.listOrganizations(tenantId) };
    },
  );
  ...
  fastify.patch<{ Params: IdParams; Body: UpdateOrganizationBody }>(
    ...
    async (request) => {
      ...
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const body = request.body;
      const updated = await request.server.coreEnterpriseService.updateOrganization(
        tenantId,
        request.params.id.trim(),
        {
          code: body.code,
  ...
  fastify.post<{ Params: IdParams }>(
    ...
    { preHandler: [requireAuth, requirePermission('organization.deactivate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const deactivated = await request.server.coreEnterpriseService.deactivateOrganization(
        tenantId,
        request.params.id.trim(),
      );
  ...
  fastify.post<{ Params: IdParams }>(
    ...
    { preHandler: [requireAuth, requirePermission('organization.activate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const activated = await request.server.coreEnterpriseService.activateOrganization(
        tenantId,
        request.params.id.trim(),
      );
      if (!activated) throw new NotFoundError('Organization not found.');
      await recordSecurityEvent(request, {
        tenantId,
        actorUserId: request.user?.id,
  ...
  fastify.delete<{ Params: IdParams }>(
    ...
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const deleted = await request.server.coreEnterpriseService.deleteOrganization(tenantId, request.params.id.trim());
  ...
  fastify.post<{ Params: OrganizationParams; Body: CreateBranchBody }>(
    ...
    { preHandler: [requireAuth, requirePermission('branch.create')] },
    async (request, reply) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const body = request.body;
      const branch = await request.server.coreEnterpriseService.createBranch(
        tenantId,
  ...
  fastify.get<{ Params: OrganizationParams }>(
    ...
    async (request) => {
      ...
      return {
        success: true,
        branches: await request.server.coreEnterpriseService.listBranches(
          tenantId,
          request.params.organizationId.trim(),
        ),
      };
  ...
  fastify.get<{ Params: OrganizationBranchParams }>(
    ...
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const branch = await request.server.coreEnterpriseService.getBranch(
        tenantId,
        request.params.organizationId.trim(),
        request.params.branchId.trim(),
      );
  ...
  fastify.patch<{ Params: OrganizationBranchParams; Body: UpdateBranchBody }>(
    '/organizations/:organizationId/branches/:branchId',
    { preHandler: [requireAuth, requirePermission('branch.update')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const body = request.body;
      const updated = await request.server.coreEnterpriseService.updateBranch(
        tenantId,
        request.params.organizationId.trim(),
        request.params.branchId.trim(),
        {
  ...
  fastify.post<{ Params: OrganizationBranchParams }>(
    ...
    { preHandler: [requireAuth, requirePermission('branch.deactivate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
  ...
  fastify.post<{ Params: OrganizationBranchParams }>(
    '/organizations/:organizationId/branches/:branchId/activate',
    { preHandler: [requireAuth, requirePermission('branch.activate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const activated = await request.server.coreEnterpriseService.activateBranch(
        tenantId,
        request.params.organizationId.trim(),
        request.params.branchId.trim(),
      );
      if (!activated) throw new NotFoundError('Branch not found.');
      return { success: true, activated: true };
    },
  );
  fastify.delete<{ Params: OrganizationBranchParams }>(
    '/organizations/:organizationId/branches/:branchId',
    { preHandler: [requireAuth, requirePermission('branch.delete')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const deleted = await request.server.coreEnterpriseService.deleteBranch(
        tenantId,
        request.params.organizationId.trim(),
        request.params.branchId.trim(),
      );
  ...
  fastify.get('/users', { preHandler: [requireAuth, requirePermission('user.read')] }, async (request) => {
    const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
    if (!tenantId) throw new ValidationError('Tenant context is required.');
    return { success: true, users: await request.server.coreEnterpriseService.listUsers(tenantId) };
  });
  fastify.get<{ Params: IdParams }>(
    '/users/:id',
    ...
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const user = await request.server.coreEnterpriseService.getUser(tenantId, request.params.id.trim());
  ...
  fastify.patch<{ Params: IdParams; Body: UpdateUserBody }>(
    ...
    async (request) => {
      ...
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const body = request.body;
      const updated = await request.server.coreEnterpriseService.updateUser(tenantId, request.params.id.trim(), {
  ...
  fastify.post<{ Params: UserBranchAccessParams }>(
    ...
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const assigned = await request.server.coreEnterpriseService.assignUserToBranch(
        tenantId,
        request.params.userId.trim(),
        request.params.branchId.trim(),
      );
  ...
  fastify.post<{ Params: IdParams }>(
    '/users/:id/activate',
    { preHandler: [requireAuth, requirePermission('user.activate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const activated = await request.server.coreEnterpriseService.activateUser(tenantId, request.params.id.trim());
  ...
  fastify.post<{ Params: IdParams }>(
    '/users/:id/deactivate',
    { preHandler: [requireAuth, requirePermission('user.deactivate')] },
    async (request) => {
      const tenantId = request.tenantId ?? getTenantIdFromRequest(request);
      if (!tenantId) throw new ValidationError('Tenant context is required.');
      const deactivated = await request.server.coreEnterpriseService.deactivateUser(tenantId, request.params.id.trim());
