...
declare module 'fastify' {
  ...
  interface FastifyRequest { user?: AuthenticatedUser; tenantId?: string; sessionId?: string; contextType?: 'tenant' | 'platform'; platformMembershipId?: string; identityId?: string; }
}
...
export async function requireAuth(request: FastifyRequest, _reply: FastifyReply): Promise<void> { const token = getBearerToken(request); if (!token) throw new UnauthorizedError('Authentication token is required.'); const claims = request.server.jwtTokenService.verifyAccessToken(token); if (!claims.tenantId || claims.contextType === 'platform') throw new UnauthorizedError('A tenant context is required.'); const session = await request.server.authService.validateSession(claims.sessionId, claims.tenantId); if (!session) throw new UnauthorizedError('Session is invalid or expired.'); request.user = session; request.tenantId = session.tenantId; request.sessionId = claims.sessionId; }
export function requirePlatformContext(permissionKey?: string) { return async function requirePlatformContextHandler(request: FastifyRequest, _reply: FastifyReply): Promise<void> { const token = getBearerToken(request); if (!token) throw new UnauthorizedError('Authentication token is required.'); const claims = request.server.jwtTokenService.verifyAccessToken(token); if (claims.contextType !== 'platform') throw new ForbiddenError('Platform context is required.'); const context = await request.server.platformAuthorizationService.validateContext(claims.sessionId, claims.sub); if (!context) throw new UnauthorizedError('Platform session is invalid or expired.'); if (permissionKey && !(await request.server.platformAuthorizationService.hasPermission(context.platformMembershipId, permissionKey))) throw new ForbiddenError('Platform permission denied.'); request.contextType = 'platform'; request.sessionId = context.sessionId; request.identityId = context.identityId; request.platformMembershipId = context.platformMembershipId; }; }
...
function moduleCodeForPermission(permissionKey: string): string { const prefix = permissionKey.split('.')[0]?.trim() ?? ''; switch (prefix) { case 'tenant': return 'tenant-configuration'; case 'user': return 'user-management'; case 'role': case 'role_permission': case 'permission': case 'session': return 'security'; case 'customer': return 'crm'; case 'sales': return 'sales'; default: return prefix; } }
...
export function requirePlatformPermission(permissionKey: string) { return async function requirePlatformPermissionHandler(request: FastifyRequest, _reply: FastifyReply): Promise<void> { if (!request.user || !request.tenantId) throw new UnauthorizedError('Authentication is required to perform this action.'); const allowed = await request.server.authorizationService.hasPermission(request.tenantId, request.user.id, permissionKey); if (!allowed) throw new ForbiddenError('Platform permission denied.'); }; }
