...
class SalesQuotationDetailsScreen extends StatefulWidget {
  ...
  @override
  State<SalesQuotationDetailsScreen> createState() =>
      _SalesQuotationDetailsScreenState();
}

class _SalesQuotationDetailsScreenState
    extends State<SalesQuotationDetailsScreen> {
  ...
  Future<void> _convertToOrder() async {
    ...
    if (inventory.error != null || inventory.warehouses.isEmpty) {
      setState(
        () => error =
            inventory.error ?? 'An active Inventory warehouse is required.',
      );
      return;
    }
    ...
    final result = await service.convertQuotationToOrder(
      widget.id,
      warehouseId,
    );
  ...
  Widget build(BuildContext context) {
    ...
    final canEdit =
        status == 'DRAFT' && auth.hasPermission('sales.quotation.update');
    final actions = <String, String>{
      if (status == 'DRAFT' && auth.hasPermission('sales.quotation.send'))
        'send': 'Send',
      if (status == 'DRAFT' && auth.hasPermission('sales.quotation.cancel'))
        'cancel': 'Cancel',
      if (status == 'SENT' && auth.hasPermission('sales.quotation.accept'))
        'accept': 'Accept',
      if (status == 'SENT' && auth.hasPermission('sales.quotation.reject'))
        'reject': 'Reject',
      if (status == 'SENT' && auth.hasPermission('sales.quotation.expire'))
        'expire': 'Expire',
      if (status == 'SENT' && auth.hasPermission('sales.quotation.cancel'))
        'cancel': 'Cancel',
    };
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          if (canEdit)
            IconButton(
              onPressed: () => Navigator.pushNamed(
                context,
                '/sales/quotations/${widget.id}/edit',
              ),
          ...
          if (auth.hasPermission('sales.quotation.delete') && status == 'DRAFT')
            IconButton(
              onPressed: () async {
                final result = await service.deleteQuotation(widget.id);
      ...
      body: ListView(
        ...
        children: [
          ...
          Text(
            'Customer: ${quotation!['customerName'] ?? quotation!['customer_name'] ?? quotation!['customerId'] ?? quotation!['customer_id']}',
          ),
          Text(
            'Quotation date: ${quotation!['quotationDate'] ?? quotation!['quotation_date']}',
          ),
          Text(
            'Valid until: ${quotation!['validUntil'] ?? quotation!['valid_until']}',
          ),
          if ((quotation!['notes'] ?? '').toString().isNotEmpty)
            Text('Notes: ${quotation!['notes']}'),
          const SizedBox(height: 16),
          ...items.map(
            (item) => ListTile(
              title: Text('${item['description']}'),
              subtitle: Text(
                '${item['quantity']} ${item['unitOfMeasure'] ?? item['unit_of_measure']} @ ${item['unitPrice'] ?? item['unit_price']}',
              ),
            ),
          ),
