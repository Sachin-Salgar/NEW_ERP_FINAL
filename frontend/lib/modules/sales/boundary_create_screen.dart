...
class SalesBoundaryCreateScreen extends StatefulWidget {
  ...
  final String title;
  const SalesBoundaryCreateScreen({super.key, required this.kind, required this.sourceLabel, required this.title});
  @override State<SalesBoundaryCreateScreen> createState() => _SalesBoundaryCreateScreenState();
}
...
class _SalesBoundaryCreateScreenState extends State<SalesBoundaryCreateScreen> {
  ...
  @override
  void dispose() {
    source.dispose(); idempotency.dispose(); notes.dispose(); super.dispose();
  }

  Future<void> _submit() async {
    setState(() { saving = true; error = null; });
    final sourceKey = widget.kind == 'returns' ? 'invoiceId' : 'returnId';
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        TextField(controller: source, decoration: InputDecoration(labelText: widget.sourceLabel)),
        TextField(controller: idempotency, decoration: const InputDecoration(labelText: 'Idempotency key')),
