...
class SalesAdminDetailsScreen extends StatefulWidget {
  final String kind;
  final String id;
  const SalesAdminDetailsScreen({
    super.key,
    required this.kind,
    required this.id,
  });
  @override
  State<SalesAdminDetailsScreen> createState() =>
      _SalesAdminDetailsScreenState();
}
...
class _SalesAdminDetailsScreenState extends State<SalesAdminDetailsScreen> {
  ...
  Future<void> load() async {
    final result = await service.getSalesAdministration(widget.kind, widget.id);
    if (mounted)
      setState(() {
        value = result;
  ...
  Future<void> addItem() async {
    final result = await service.addPriceListItem(widget.id, {
  ...
  Future<void> updateDiscount() async {
    final result = await service.updateDiscountRule(widget.id, {
      'name': value?['name'],
      'percentage': double.tryParse(percentage.text),
      'effectiveFrom': value?['effectiveFrom'],
      'effectiveTo': value?['effectiveTo'],
      'expectedVersion': value?['versionNumber'],
    });
    if (!mounted) return;
    if (result == null)
      load();
  ...
  Widget build(BuildContext context) {
    if (value == null)
      return Scaffold(
        appBar: AppBar(title: const Text('Sales administration')),
        body: Center(
          child: error == null
    ...
    return Scaffold(
      appBar: AppBar(title: Text('${value!['name'] ?? value!['code'] ?? ''}')),
      body: ListView(
        ...
        children: [
          ...
          if (isPricing) ...[
            ...
            ...((value!['items'] as List<dynamic>?) ?? const []).map(
              (item) => ListTile(
                title: Text('${item['itemCode'] ?? ''}'),
                subtitle: Text('${item['unitOfMeasure'] ?? ''}'),
                trailing: Text('${item['price'] ?? ''}'),
              ),
            ),
            if (status == 'DRAFT' &&
                auth.hasPermission('$permission.update')) ...[
              TextField(
                controller: itemCode,
          ...
          if (status == 'DRAFT' && auth.hasPermission('$permission.publish'))
            FilledButton(
              onPressed: () async {
                final e = await service.transitionSalesAdministration(
                  widget.kind,
                  widget.id,
                  'publish',
                  (value!['versionNumber'] as num?)?.toInt() ?? 1,
                );
                if (e == null) load();
              },
          ...
          if (status == 'PUBLISHED' &&
              auth.hasPermission('$permission.archive'))
            OutlinedButton(
              onPressed: () async {
                final e = await service.transitionSalesAdministration(
                  widget.kind,
