...
class _OrganizationListScreenState extends State<OrganizationListScreen> {
  ...
  void initState() {
    super.initState();
    final api = ApiClient(baseUrl: const String.fromEnvironment('API_BASE_URL', defaultValue: 'http://localhost:3001'));
    ...
    return ChangeNotifierProvider<OrganizationService>.value(
      ...
      child: Consumer<OrganizationService>(
        builder: (context, svc, _) {
          ...
          if (!permitted) return Center(child: Text('You do not have permission to view organizations.'));
          ...
          return Scaffold(
            ...
            body: RefreshIndicator(
              ...
              child: items.isEmpty
                  ? ListView(children: [Center(child: Padding(padding: EdgeInsets.all(24), child: Text('No organizations found.')))])
                  : ListView.builder(
                      itemCount: items.length,
                      itemBuilder: (context, index) {
                        final org = items[index];
                        return ListTile(
                          title: Text(org['name'] ?? org['code'] ?? 'Unnamed'),
                          subtitle: Text(org['legalName'] ?? ''),
                          onTap: () => Navigator.of(context).pushNamed('/organizations/details', arguments: org['id']),
                        );
                      },
                    ),
            ),
            floatingActionButton: auth.hasPermission('organization.manage')
                ? FloatingActionButton(
                    onPressed: () => Navigator.of(context).pushNamed('/organizations/create'),
