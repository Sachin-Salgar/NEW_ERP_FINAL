...
class _PlatformAdministrationScreenState
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        _policy = Map<String, dynamic>.from(
          (r[5]['policy'] as Map?) ?? const {},
        );
      ...
      builder: (dialog) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                _field(name, 'Tenant name *'),
                _field(display, 'Display name'),
                _field(subdomain, 'Subdomain *'),
                _field(slug, 'Slug *'),
                _field(branch, 'Default branch *'),
  ...
  Future<void> _manageModules(Map<String, dynamic> tenant) async {
    try {
      final data = await _get(
        '/api/v1/platform/tenants/' + tenant['id'] + '/modules',
      );
          ...
          builder: (context, setDialogState) => AlertDialog(
            ...
            content: SizedBox(
              ...
              child: ListView(
                children: [
                  const Text(
                    'Core modules are mandatory. Optional modules can be enabled or disabled for this tenant.',
                  ),
                  const SizedBox(height: 12),
        ...
        child: Wrap(
          children: [
            ListTile(
              ...
              onTap: () => Navigator.pop(sheet, 'modules'),
            ),
            ListTile(
            ...
            if (status == 'active')
              ListTile(
                ...
                onTap: () => Navigator.pop(sheet, 'suspend'),
              ),
            if (status == 'suspended')
              ListTile(
                leading: const Icon(Icons.play_circle_outline),
                title: const Text('Reactivate'),
                onTap: () => Navigator.pop(sheet, 'reactivate'),
              ),
            if (status != 'active' && status != 'cancelled')
              ListTile(
                leading: const Icon(Icons.play_arrow_outlined),
                title: const Text('Activate'),
                onTap: () => Navigator.pop(sheet, 'activate'),
              ),
            ...
            children: [
              for (final identity in _identities)
                ListTile(
                  ...
                  subtitle: Text(
                    identity['email']?.toString() ?? identity['id'].toString(),
                  ),
  ...
  Future<void> _editRole(Map<String, dynamic> role) async {
    if (role['isSystem'] == true) {
      _errorSnack('System role permissions are protected.');
      return;
    }
              ...
              onPressed: () async {
                try {
                  final response = await _client.put(
                    ...
                    body: {'permissionIds': selected.toList()},
                  );
                  if (response.statusCode < 200 || response.statusCode >= 300)
                    throw Exception(response.body);
                  if (dialog.mounted) Navigator.pop(dialog, true);
  ...
  Future<void> _editPolicy() async {
    final session = TextEditingController(
      text: (_policy['sessionLifetimeMinutes'] ?? 60).toString(),
    );
    final failed = TextEditingController(
      text: (_policy['maxFailedLoginAttempts'] ?? 5).toString(),
    );
    final lockout = TextEditingController(
      text: (_policy['lockoutMinutes'] ?? 15).toString(),
    );
  ...
  Widget _tenants() => _section(
    ...
    subtitle:
        'Create, configure, activate, suspend and manage tenant entitlements.',
            ...
            child: ListTile(
              ...
              subtitle: Text(
                (tenant['slug']?.toString() ?? '') +
                    ' • ' +
                    (tenant['subdomain']?.toString() ?? '') +
                    '\nStatus: ' +
                    (tenant['status']?.toString() ?? 'unknown'),
              ),
              isThreeLine: true,
              trailing: FilledButton.tonalIcon(
                onPressed: () => _tenantActions(tenant),
          ...
          Card(
            child: ListTile(
              ...
              trailing: FilledButton.tonal(
                onPressed: () => _manageModules(tenant),
          ...
          Card(
            child: ListTile(
              ...
              subtitle: Text(
                (role['code']?.toString() ?? '') +
                    ' • ' +
                    ((role['permissions'] as List<dynamic>? ?? const []).length
                        .toString()) +
                    ' permissions',
              ),
              ...
              _section(
                ...
                child: Wrap(
                  ...
                  children: [
                    _stat(
                      'Tenants',
                      _tenantData.length.toString(),
                      Icons.business_outlined,
                    ),
                    _stat(
                      'Platform members',
                      _memberData.length.toString(),
                      Icons.people_outline,
                    ),
                    _stat(
                      'Platform roles',
                      _roleData.length.toString(),
                      Icons.admin_panel_settings_outlined,
                    ),
                    _stat(
                      'Permissions',
