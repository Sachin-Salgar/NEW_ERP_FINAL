...
class ManufacturingScreen extends StatefulWidget { const ManufacturingScreen({super.key}); @override State<ManufacturingScreen> createState()=>_ManufacturingScreenState(); }
...
class _ManufacturingScreenState extends State<ManufacturingScreen> {
 ...
 Widget _capabilities()=>ListView(children:[
  ...
  Row(children:[Expanded(child:action('Add Capability',Icons.add,()=>s.createCapability({'machineId':machineId.text,'operationCode':opCode.text,'capabilityName':name.text}))),const SizedBox(width:12),OutlinedButton.icon(onPressed:()=>s.refresh(),icon:const Icon(Icons.refresh),label:const Text('Refresh'))]),
  const SizedBox(height:20),...s.capabilities.map((x)=>Card(child:ListTile(title:Text((x['operation_code']??'').toString()+' — '+(x['capability_name']??'').toString()),subtitle:Text((x['machine_code']??'').toString()+' / '+(x['machine_name']??'').toString()),trailing:Text((x['status']??'').toString())))),
 ]);
 Widget _process()=>ListView(children:[
  const Text('FEAT-011 — Product Process / Routing',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:16),
  f('Inventory Item ID',itemId),f('Revision',revision),f('Drawing Number',result),
  Row(children:[Expanded(child:action('Create Process Detail',Icons.route,()=>s.createProcess({'itemId':itemId.text,'revision':revision.text,'drawingNumber':result.text}))),const SizedBox(width:12),Expanded(child:action('Add Routing Operation',Icons.alt_route,()=>s.addRouting(processId.text,{'sequenceNo':1,'operationCode':opCode.text,'operationDescription':name.text}))) ]),
  const SizedBox(height:10),const Text('Routing supports operation sequence, machine group/code, manpower, cycle/setup/allowance/overhead times, batch quantity, tooling, work instructions and process parameters through the API.',style:TextStyle(color:Colors.grey)),
 ]);
 Widget _workOrders()=>ListView(children:[
  const Text('FEAT-001 / FEAT-002 — Work Order, Scheduling & Route Card',style:TextStyle(fontSize:20,fontWeight:FontWeight.bold)),const SizedBox(height:16),
 ...
 Widget _execution()=>ListView(children:[
  ...
  f('Task Sheet ID',taskId),f('Machine ID',machineId),f('Tool ID (optional)',toolId),f('Fixture ID (optional)',fixtureId),
  action('Run Readiness Gate',Icons.fact_check,()=>s.readiness({'taskSheetId':taskId.text,'machineId':machineId.text,'toolId':toolId.text.isEmpty?null:toolId.text,'fixtureId':fixtureId.text.isEmpty?null:fixtureId.text})),
  const Divider(height:32),f('Quantity',qty),f('Operation Key',operationKey),f('Return Number',returnNo),
  Wrap(spacing:8,runSpacing:8,children:[
   action('Punch Good Output',Icons.check_circle,()=>s.punchProduction({'taskSheetId':taskId.text,'quantity':double.tryParse(qty.text)??0,'outputType':'GOOD','operationKey':operationKey.text})),
