...
class _SecurityAdministrationScreenState
  ...
  Future<void> _load() async {
    try {
      ...
      final values = {
        'actorUserId': _actor.text,
        'action': _action.text,
        'resourceType': _resourceType.text,
        'resourceId': _resourceId.text,
        'correlationId': _correlation.text,
      };
      values.forEach((k, v) {
      ...
      final results = await Future.wait([
        ...
        _api.get('/api/v1/security/audit-logs?$qs'),
      ]);
      if (!mounted) return;
      ...
      setState(() {
        ...
        _logs = (audit['logs'] as List<dynamic>?) ?? [];
        _metadata = (audit['metadata'] as Map<String, dynamic>?) ?? {};
        _error = null;
      });
    } catch (e) {
  ...
  Future<void> _revoke(String id) async {
    final r = await _api.post('/api/v1/security/sessions/$id/revoke');
  ...
  Future<void> _exportAudit() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          r.statusCode < 400 ? 'Audit export generated.' : _message(r),
        ),
      ),
    );
  }
  ...
  Future<void> _editPolicy() async {
    ...
    final session = TextEditingController(
      text: '${_policy!['sessionLifetimeMinutes']}',
    );
    final failed = TextEditingController(
      text: '${_policy!['maxFailedLoginAttempts']}',
    );
    final lockout = TextEditingController(
      text: '${_policy!['lockoutMinutes']}',
    );
    ...
    final result = await showDialog<bool>(
      ...
      builder: (c) => StatefulBuilder(
        builder: (c, setDialog) => AlertDialog(
          title: const Text('Edit Security Policy'),
            ...
            child: Column(
              ...
              children: [
                ...
                TextField(
                  ...
                  keyboardType: TextInputType.number,
                  decoration: const InputDecoration(
                    labelText: 'Maximum failed login attempts',
                  ),
                ),
                TextField(
                  controller: lockout,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          r.statusCode < 400 ? 'Security policy updated.' : _message(r),
        ),
      ),
    );
  ...
  Widget _filters() => Card(
    child: Padding(
      ...
      child: Wrap(
        ...
        runSpacing: 12,
        children: [
          _filter(_actor, 'Actor user ID'),
          _filter(_action, 'Action'),
          _filter(_resourceType, 'Resource type'),
          _filter(_resourceId, 'Resource ID'),
          _filter(_correlation, 'Correlation ID'),
  ...
  Widget _filter(TextEditingController c, String label) => SizedBox(
    width: 190,
    child: TextField(
      controller: c,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Security Administration'),
        actions: [
          IconButton(onPressed: _editPolicy, icon: const Icon(Icons.tune)),
        ...
        child: ListView(
          ...
          children: [
            ...
            ListTile(
              title: const Text('Require MFA'),
              subtitle: Text(
                _policy!['mfaRequired'] == true ? 'Enabled' : 'Disabled',
              ),
            ),
            ListTile(
            ...
            ..._sessions.map(
              (s) => ListTile(
                title: Text(s['userId']?.toString() ?? 'Session'),
                subtitle: Text(s['loginAt']?.toString() ?? ''),
                trailing: Wrap(
                  children: [
                    IconButton(
                      icon: const Icon(Icons.logout),
                      onPressed: () => _revoke(s['id'].toString()),
                    ),
                    IconButton(
                      icon: const Icon(Icons.logout_outlined),
                      onPressed: () => _revokeAll(s['userId'].toString()),
                    ),
                  ],
                ),
              ),
