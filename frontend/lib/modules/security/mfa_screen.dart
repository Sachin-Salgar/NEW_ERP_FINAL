...
class _MfaScreenState extends State<MfaScreen> {
  ...
  Future<void> _begin() async {
    ...
    try {
      ...
      setState(
        () => _enrollment = Map<String, dynamic>.from(
  ...
  Future<void> _confirm() async {
    ...
    try {
      ...
      setState(() {
        _recovery = (body['recoveryCodes'] as List<dynamic>?) ?? [];
        _enrollment = null;
        _code.clear();
        _message =
            'MFA enrollment confirmed. Store the recovery codes securely.';
      });
    } catch (e) {
      setState(() => _message = e.toString().replaceFirst('Exception: ', ''));
    } finally {
  ...
  Future<void> _verifyMfa() async {
    ...
    if (token.length < 6) {
      setState(() => _message = 'Enter an MFA or recovery code.');
      return;
    }
    setState(() => _loading = true);
    try {
      ...
      if (response.statusCode >= 400) throw Exception(_error(response));
      setState(
        () => _message = jsonDecode(response.body)['verified'] == true
            ? 'MFA verification succeeded.'
            : 'MFA code was not accepted.',
      );
    } catch (e) {
      setState(() => _message = e.toString().replaceFirst('Exception: ', ''));
    } finally {
  ...
  Future<void> _disable() async {
    final ok = await showDialog<bool>(
      ...
      builder: (c) => AlertDialog(
        ...
        content: const Text(
          'This removes MFA protection for your current account.',
        ),
        actions: [
          TextButton(
    ...
    if (!mounted) return;
    setState(
      () => _message = response.statusCode < 400
          ? 'MFA disabled.'
          : _error(response),
    );
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: ListView(
      ...
      children: [
        ...
        const Text(
          'Generate a TOTP enrollment secret, add it to your authenticator app, then confirm the 6-digit code.',
        ),
        const SizedBox(height: 16),
        TextField(
          controller: _label,
          decoration: const InputDecoration(
            labelText: 'Authenticator account label (optional)',
          ),
        ),
        ...
        FilledButton.icon(
          ...
          label: const Text('Begin MFA enrollment'),
        ),
        if (_enrollment != null) ...[
          const SizedBox(height: 16),
          Card(
          ...
          TextField(
            ...
            maxLength: 6,
            decoration: const InputDecoration(
              labelText: '6-digit enrollment code',
            ),
          ),
          FilledButton(
        ...
        if (_recovery.isNotEmpty)
          Card(
            child: Padding(
              padding: const EdgeInsets.all(16),
              child: SelectableText('Recovery codes:\n${_recovery.join('\n')}'),
            ),
          ),
        ...
        TextField(
          controller: _verify,
          decoration: const InputDecoration(labelText: 'MFA / recovery code'),
        ),
        const SizedBox(height: 12),
        FilledButton(
