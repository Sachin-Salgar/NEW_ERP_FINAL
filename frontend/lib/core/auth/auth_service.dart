...
class AuthService extends ChangeNotifier {
  ...
  String? currentTenantId, currentOrganizationId, currentBranchId, currentLocationId;
  String? selectedOrganizationId, selectedBranchId, selectedLocationId;
  Map<String, dynamic>? currentUser, deploymentInfo;
  List<Map<String, dynamic>> availableOrganizations = const [], availableBranches = const [], availableLocations = const [], availableModules = const [];
  bool requiresOrganizationSelection = false, requiresBranchSelection = false, requiresLocationSelection = false;
  ...
  Future<void> init() async {
    ...
    currentTenantId = await _secureStorage.read(key: 'tenant_id'); currentOrganizationId = await _secureStorage.read(key: 'organization_id'); currentBranchId = await _secureStorage.read(key: 'branch_id'); currentLocationId = await _secureStorage.read(key: 'location_id');
    selectedOrganizationId = currentOrganizationId; selectedBranchId = currentBranchId; selectedLocationId = currentLocationId; if (_accessToken != null) notifyListeners();
  }
  ...
  Future<void> _storeSession(Map<String,dynamic> body) async {
    ...
    final branch = (s['branchId'] ?? currentUser?['defaultBranchId'] ?? '').toString().trim(); currentBranchId = branch.isEmpty ? null : branch;
    final loc = (s['locationId'] ?? currentUser?['activeLocationId'] ?? currentUser?['defaultLocationId'] ?? '').toString().trim(); currentLocationId = loc.isEmpty ? null : loc; selectedOrganizationId = currentOrganizationId; selectedBranchId = currentBranchId; selectedLocationId = currentLocationId;
    ...
    if (currentTenantId != null) await _secureStorage.write(key:'tenant_id',value:currentTenantId!); if (currentOrganizationId != null) await _secureStorage.write(key:'organization_id',value:currentOrganizationId!); if (currentBranchId != null) await _secureStorage.write(key:'branch_id',value:currentBranchId!); if (currentLocationId != null) await _secureStorage.write(key:'location_id',value:currentLocationId!);
  }

  Future<bool> loadMe(String baseUrl) async { if (_accessToken == null) return false; _ensureApiClient(baseUrl); try { final r=await _apiClient.get('/api/v1/auth/me'); if(r.statusCode!=200)return false; final b=jsonDecode(r.body) as Map<String,dynamic>; currentUser=b['user'] as Map<String,dynamic>?; currentTenantId=(currentUser?['tenantId'] ?? currentTenantId)?.toString(); currentOrganizationId=(currentUser?['organizationId'] ?? currentOrganizationId)?.toString(); currentBranchId=(currentUser?['defaultBranchId'] ?? currentBranchId)?.toString(); currentLocationId=(currentUser?['activeLocationId'] ?? currentUser?['defaultLocationId'] ?? currentLocationId)?.toString(); selectedOrganizationId = currentOrganizationId; selectedBranchId = currentBranchId; selectedLocationId = currentLocationId; notifyListeners(); return true; } catch(_){return false;} }
  ...
  Future<bool> loadAuthorizedBranches(String baseUrl) async {
    ...
    try {
      ...
      availableBranches = branches.map((item) => Map<String, dynamic>.from(item as Map)).toList();
      final active = (currentBranchId ?? selectedBranchId ?? '').toString().trim();
      requiresBranchSelection = false;
      if (active.isNotEmpty && availableBranches.any((branch) => (branch['id'] ?? '').toString() == active)) {
  ...
  Future<bool> selectOrganization(String organizationId) async {
    ...
    try {
      ...
      final resolvedBranchId = (availableBranches.any((branch) => (branch['id'] ?? '').toString() == (selectedBranchId ?? currentBranchId ?? '')) ? (selectedBranchId ?? currentBranchId) : (availableBranches.isNotEmpty ? (availableBranches.first['id'] ?? '').toString() : null));
      final resolvedLocationId = (availableLocations.any((location) => (location['id'] ?? '').toString() == (selectedLocationId ?? currentLocationId ?? '')) ? (selectedLocationId ?? currentLocationId) : (availableLocations.isNotEmpty ? (availableLocations.first['id'] ?? '').toString() : null));
      ...
      final ok = await applyWorkingContext(organizationId: id, branchId: resolvedBranchId, locationId: resolvedLocationId);
      if (!ok) {
        currentOrganizationId = previousOrg;
        selectedOrganizationId = previousOrg;
        currentBranchId = previousBranch;
  ...
  Future<bool> applyWorkingContext({required String organizationId, required String branchId, required String locationId}) async {
    final base = _currentBaseUrl;
    _ensureApiClient(base);
    try {
      final r = await _apiClient.post('/api/v1/auth/context/select', body: {'organizationId': organizationId, 'branchId': branchId, 'locationId': locationId});
      if (r.statusCode != 200) return false;
      await _storeSession(jsonDecode(r.body) as Map<String, dynamic>);
  ...
  Future<bool> loadAuthorizedLocations(String baseUrl) async { if(_accessToken==null||currentOrganizationId==null)return false; _currentBaseUrl=baseUrl;_ensureApiClient(baseUrl);try{final r=await _apiClient.get('/api/v1/locations');if(r.statusCode!=200)return false;final b=jsonDecode(r.body) as Map<String,dynamic>;final l=(b['locations'] as List<dynamic>?)??const[];availableLocations=l.map((x)=>Map<String,dynamic>.from(x as Map)).toList();final active=(currentLocationId??selectedLocationId??'').toString().trim();requiresLocationSelection=false;if(active.isNotEmpty&&availableLocations.any((x)=>(x['id']??'').toString()==active)){currentLocationId=active;selectedLocationId=active;await _secureStorage.write(key:'location_id',value:active);} else if (availableLocations.isNotEmpty) { final fallback=(availableLocations.first['id']??'').toString(); if(fallback.isNotEmpty){currentLocationId=fallback;selectedLocationId=fallback;await _secureStorage.write(key:'location_id',value:fallback);} } else { currentLocationId=null;selectedLocationId=null;await _secureStorage.delete(key:'location_id'); } notifyListeners();return true;}catch(_){return false;} }
  Future<bool> selectBranch(String branchId) async {final id=branchId.trim();if(id.isEmpty||!availableBranches.any((o)=>(o['id']??'').toString()==id))return false;final orgId=currentOrganizationId ?? selectedOrganizationId;final preferredLocationId = (availableLocations.any((l)=>(l['id']??'').toString() == (selectedLocationId ?? currentLocationId ?? '')) ? (selectedLocationId ?? currentLocationId) : (availableLocations.isNotEmpty ? (availableLocations.first['id'] ?? '').toString() : null));if(orgId==null||preferredLocationId==null)return false;return applyWorkingContext(organizationId: orgId, branchId: id, locationId: preferredLocationId); }
  Future<bool> selectLocation(String locationId) async {final id=locationId.trim();if(id.isEmpty||!availableLocations.any((o)=>(o['id']??'').toString()==id))return false;final orgId=currentOrganizationId ?? selectedOrganizationId;final preferredBranchId = (availableBranches.any((b)=>(b['id']??'').toString() == (selectedBranchId ?? currentBranchId ?? '')) ? (selectedBranchId ?? currentBranchId) : (availableBranches.isNotEmpty ? (availableBranches.first['id'] ?? '').toString() : null));if(orgId==null||preferredBranchId==null)return false;return applyWorkingContext(organizationId: orgId, branchId: preferredBranchId, locationId: id); }
  ...
  Future<void> logout() async {try{if(_accessToken!=null){_ensureApiClient(_currentBaseUrl);await _apiClient.post('/api/v1/auth/logout');}}catch(_){} _refreshRequest = null; _accessToken=null;_refreshToken=null;_expiresAt=null;currentTenantId=null;currentOrganizationId=null;currentBranchId=null;currentLocationId=null;selectedOrganizationId=null;selectedBranchId=null;selectedLocationId=null;currentUser=null;availableOrganizations=const[];availableBranches=const[];availableLocations=const[];availableModules=const[];requiresOrganizationSelection=false;requiresBranchSelection=false;requiresLocationSelection=false;authzService.clear();for(final k in ['access_token','refresh_token','expires_at','tenant_id','organization_id','branch_id','location_id'])await _secureStorage.delete(key:k);notifyListeners();}
}
