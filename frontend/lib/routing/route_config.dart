...
class AppRoutes {
  ...
  static const organizations = AppRouteConfig(path: '/organizations', title: 'Organizations', group: 'MANAGEMENT', permissionKey: 'organization.read', moduleCode: 'organization', icon: Icons.apartment_outlined);
  static const branches = AppRouteConfig(path: '/organizations/branches', title: 'Branches', group: 'MANAGEMENT', permissionKey: 'branch.read', moduleCode: 'branch', icon: Icons.store_outlined);
  static const settingsOrganizations = AppRouteConfig(path: '/settings/organizations', title: 'Organizations', group: 'SETTINGS', permissionKey: 'organization.read', moduleCode: 'organization', icon: Icons.apartment_outlined);
  ...
  static const routePermissions = <String, String?>{
    ...
    '/settings': null, '/platform': 'platform.tenant.read', '/settings/organizations': 'organization.read', '/settings/organizations/create': 'organization.create', '/settings/organizations/details': 'organization.read', '/settings/organizations/edit': 'organization.update',
  ...
  static const _legacyAliases = <String, String>{
    '/organizations': '/settings/organizations', '/organizations/create': '/settings/organizations/create', '/organizations/details': '/settings/organizations/details', '/organizations/edit': '/settings/organizations/edit',
    '/organizations/branches': '/settings/branches', '/organizations/branches/create': '/settings/branches/create', '/organizations/branches/details': '/settings/branches/details', '/organizations/branches/edit': '/settings/branches/edit',
  ...
  static String canonicalTopLevel(String route) { final normalized = normalize(route); for (final config in topLevel) { if (config.matches(normalized)) return config.path; } if (isSettingsRoute(normalized)) return settings.path; for (final config in [organizations, branches, customers]) { if (config.matches(normalized)) return config.path; } return dashboard.path; }
  static AppRouteConfig forRoute(String route) { final normalized = normalize(route); for (final config in settingsNavigation) { if (config.matches(normalized)) return config; } for (final config in topLevel) { if (config.matches(normalized)) return config; } for (final config in [organizations, branches, customers]) { if (config.matches(normalized)) return config; } return dashboard; }
}
