...
**Targets** (structured SLOs — owners to define numeric values):

| SLO / Metric | Target (value) | Deployment Tier | Owner | Notes |
|--------------|----------------|-----------------|-------|-------|
| p95 latency (common operations) | TBD | Production / Staging / Dev | SRE (TBD) | Define query classes in appendix
| p99 latency (common operations) | TBD | Production / Staging / Dev | SRE (TBD) | Include read and write paths
| Concurrent active users (session count) | TBD | Production | Capacity Team (TBD) | Peak sustained users
| Transaction throughput (TPS) | TBD | Production | Capacity Team (TBD) | Measured as committed transactions/sec
| Backup RPO | TBD | Production | DBA (TBD) | See docs/03-database/17-backup-recovery.md
| Backup RTO | TBD | Production | DBA (TBD) | See docs/03-database/17-backup-recovery.md

**Guidance**: Owners must replace TBD values and publish per-deployment tier SLOs. SLOs must be reviewed by the Architecture Review Board prior to becoming contractual.
