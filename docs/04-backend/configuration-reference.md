...
| `DATABASE_SSL_CA` | string | No | — | Optional PEM certificate chain for managed PostgreSQL TLS validation. |
...
- When `DATABASE_SSL_MODE=require` and the managed provider's CA is not in the runtime trust store, provide `DATABASE_SSL_CA`; certificate validation remains enabled.
