...
At the current audited implementation point, the canonical Workflow/BPM foundation is implemented with the bounded Purchase Order integration described by this ADR. The foundation currently covers versioned definitions, publication, tenant/branch-scoped instances and tasks, role-based approval assignment, configurable required approval counts for the current sequential-level model, approve/reject/correction decisions, requester self-approval prevention, decision audit, delegation persistence/API foundation, escalation trigger/scheduling foundation, notification hooks, idempotency/concurrency controls, and a module callback/handler mechanism.

The following capabilities remain future or partial extensions and are not claimed as fully implemented by this ADR: parallel approval levels, general condition/rule evaluation beyond the current definition shape, dynamic approver resolution, complete delegation execution semantics, complete escalation/SLA processing, affected-level restart, cancellation policy execution, and fully atomic document/workflow state changes across independent module transaction boundaries.

Repository code also contains a Manufacturing scheduling call into the canonical workflow service. This is recorded as an implementation conformance item, not as an authorized completed integration under this ADR: its end-to-end approval completion callback is not yet documented and verified as a bounded module integration. Sales Return has no equivalent canonical workflow integration in the audited implementation. New module workflow integration must therefore be treated as a bounded vertical slice requiring explicit architectural authorization and implementation evidence.
