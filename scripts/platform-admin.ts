...
try {
  ...
  const configured = await client.query<{ used_at: Date | null }>(
  ...
  if (operation === 'bootstrap') {
    const existing = await client.query(`SELECT 1 FROM platform_memberships WHERE status = 'active' LIMIT 1`);
    if (existing.rowCount) throw new Error('Platform administration is already initialized.');
  }
  let identityId: string;
  if (operation === 'recover') {
    const existing = await client.query<{ identityId: string }>(
    ...
    const identity = await client.query<{ id: string }>(`INSERT INTO identities (status) VALUES ('active') RETURNING id`);
