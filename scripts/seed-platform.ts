...
const lockKey = 'platform-admin-seed-v1';

try {
  await client.query('BEGIN');
  await client.query('SELECT pg_advisory_xact_lock(hashtext($1))', [lockKey]);

  const existingPlatform = await client.query<{ identity_id: string; membership_id: string }>(
    `SELECT identity_id, id AS membership_id
  ...
  if (usernameIdentifier.rowCount === 1) {
    ...
    if (existingPlatform.rowCount === 1) {
      throw new Error(
        'A different active platform administrator already exists. Do not create a second default platform administrator automatically.',
      );
    }
